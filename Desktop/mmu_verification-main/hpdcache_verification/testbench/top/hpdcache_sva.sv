...
`endif
 ...
 (  @( posedge clk_i)
  ...
  genvar i;
  generate
    for ( i = 0 ; i < hpdcacheCfg.u.nRequesters -1 ; i++ ) begin
  ...
  // ========================================================================
  // FIX 1: Convert unpacked array to packed vector for $onehot0
  // ========================================================================
  logic [hpdcacheCfg.u.nRequesters-1:0] core_req_ready_packed;
  
  always_comb begin
    for (int k = 0; k < hpdcacheCfg.u.nRequesters; k++) begin
      core_req_ready_packed[k] = core_req_ready_o[k];
    end
  end

  // Ready is one hot 
  rr_aribter_read_onehot_check : assert property ( @(posedge clk_i) disable iff(!rst_ni)
      ($onehot0(core_req_ready_packed) == 1)) else
  ...
  //sequence hpdcache_req(input hpdcache_req_op_t op);
  //  arb_req_valid & arb_req_ready & (arb_req.op == op);
  //endsequence
  //sequence hpdcache_amo_req(input hpdcache_req_op_t op);
  //  arb_req_valid & arb_req_ready & (is_amo(arb_req.op));
  //endsequence

  sequence hpdcache_req(hpdcache_req_op_t op);
    arb_req_valid & arb_req_ready & (arb_req.op == op);
  endsequence

  sequence hpdcache_amo_req(hpdcache_req_op_t op);
    arb_req_valid & arb_req_ready & (is_amo(arb_req.op));
  endsequence
  ...
  // --------------------------------------------------------
  property load_no_rsp_hpdcache_req_with_rsp(hpdcache_req_op_t Op);
    hpdcache_set_t set; 
    hpdcache_tag_t tag; 
    @( posedge clk_i )
  ...
  // --------------------------------------------------------
  property store_no_rsp_hpdcache_req_with_rsp(hpdcache_req_op_t Op);
    hpdcache_set_t set; 
    hpdcache_tag_t tag; 
    @( posedge clk_i )
  ...
  property b2b_op_op_load(hpdcache_req_op_t Op1, hpdcache_req_op_t Op2);
  ...
  property b2b_amo_op_load(hpdcache_req_op_t Op1, hpdcache_req_op_t Op2);
    hpdcache_req_addr_t       Op1_addr; 
    @( posedge clk_i )
...
// White Box
  ...
  generate 
        for ( genvar ii = 0 ; ii < hpdcacheCfg.u.nRequesters -1 ; ii++ ) begin
          ...
          for(clk_delay_itr=0; clk_delay_itr<= 5 ; clk_delay_itr++) begin: clk_delay_itr_rst_flush
              ...
              property core_req_reset_schmoo;
                ...
                $rose((core_req_valid_i[ii] & core_req_ready_o[ii]))   |-> ##clk_delay_itr $rose(rst_ni) ;
              ...
              property core_reset_req_schmoo;
                ...
                $rose(rst_ni)  |-> ##clk_delay_itr $rose((core_req_valid_i[ii] & core_req_ready_o[ii])) ;
              ...
              property core_rsp_reset_schmoo;
                ...
                $rose(core_rsp_valid_o[ii])   |-> ##clk_delay_itr $rose(rst_ni) ;
              ...
              property core_reset_rsp_schmoo;
                ...
                $rose(rst_ni)  |-> ##clk_delay_itr $rose(core_rsp_valid_o[ii]) ;
              ...
              property core_req_flush_schmoo;
                ...
                $rose((core_req_valid_i[ii] & core_req_ready_o[ii]))   |-> ##clk_delay_itr $rose(wbuf_flush_i) ;
              ...
              property core_flush_req_schmoo;
                ...
                $rose(wbuf_flush_i)  |-> ##clk_delay_itr $rose((core_req_valid_i[ii] & core_req_ready_o[ii])) ;
              ...
              property core_rsp_flush_schmoo;
                ...
                $rose(core_rsp_valid_o[ii])   |-> ##clk_delay_itr $rose(wbuf_flush_i) ;
              ...
              property core_flush_rsp_schmoo;
                ...
                $rose(wbuf_flush_i)  |-> ##clk_delay_itr $rose(core_rsp_valid_o[ii]) ;
 ...
 endgenerate
  ...
  // ========================================================================
  // FIX 2: Use localparam to convert enum values for use in properties
  // ========================================================================
  
  // 1. AMO -> OP -> LOAD (use direct enum values with localparam)
  generate 
    localparam hpdcache_req_op_t AMO_OPS[3] = '{
      HPDCACHE_REQ_AMO_LR,
      HPDCACHE_REQ_AMO_SC,
      HPDCACHE_REQ_AMO_SWAP
    };
    
    localparam hpdcache_req_op_t ALL_OPS[4] = '{
      HPDCACHE_REQ_LOAD,
      HPDCACHE_REQ_STORE,
      HPDCACHE_REQ_AMO_LR,
      HPDCACHE_REQ_AMO_SC
    ...
    for(genvar j = 0; j < 3; j++) begin : amo_op_loop
      for(genvar ii = 0; ii < 4; ii++) begin: b2b_amo_op_load_itr
        localparam hpdcache_req_op_t OP_J = AMO_OPS[j];
        localparam hpdcache_req_op_t OP_I = ALL_OPS[ii];
        
        b2b_amo_op_load_cov : cover property ( b2b_amo_op_load(OP_J, OP_I) );
      end
    end
    
    // 2. AMO -> N intervening ops -> LOAD
    for(genvar j = 0; j < 3; j++) begin : amo_n_op_loop
      for(genvar k = 0; k < 16; k++) begin: b2b_amo_n_op_load_itr
        localparam int REPEAT_COUNT = k;
        localparam hpdcache_req_op_t OP_J = AMO_OPS[j];
        
        property b2b_amo_n_op_load_internal;
          hpdcache_req_addr_t Op1_addr;
          @( posedge clk_i )
          (hpdcache_amo_req(OP_J), Op1_addr = addr_sva_aligned) 
          ##1 ((arb_req_valid & arb_req_ready) & (addr_sva_aligned != Op1_addr))[=REPEAT_COUNT]
          ##1 hpdcache_req(HPDCACHE_REQ_LOAD) ##0 (addr_sva_aligned == Op1_addr);
        endproperty : b2b_amo_n_op_load_internal 
        
        b2b_amo_n_op_load_cov: cover property ( b2b_amo_n_op_load_internal );
      end
    end

    // 3. STORE -> OP -> LOAD
    for(genvar ii = 0; ii < 4; ii++) begin: b2b_op_op_load_itr
      localparam hpdcache_req_op_t OP_I = ALL_OPS[ii];
      b2b_write_op_load_cov: cover property ( b2b_op_op_load(HPDCACHE_REQ_STORE, OP_I) );
    end
  endgenerate      

  // 4. STORE -> N intervening ops -> LOAD
  generate 
    for(genvar k = 0; k < 16; k++) begin: b2b_op_n_op_load_itr
      localparam int REPEAT_COUNT = k;
      
      property b2b_op_n_op_load_internal;
        hpdcache_req_addr_t Op1_addr;
        @( posedge clk_i )
        (hpdcache_req(HPDCACHE_REQ_STORE), Op1_addr = addr_sva_aligned) 
        ##1 ((arb_req_valid & arb_req_ready) & (addr_sva_aligned != Op1_addr))[=REPEAT_COUNT]
        ##1 hpdcache_req(HPDCACHE_REQ_LOAD) ##0 (addr_sva_aligned == Op1_addr);
      endproperty
      
      b2b_write_n_op_load_cov: cover property ( b2b_op_n_op_load_internal );
    end
  endgenerate 
  ...
  function int get_index();
    automatic int idx = -1;
    for(int iii = 0; iii < hpdcacheCfg.u.nRequesters; iii ++) begin
      if( core_req_valid_i[iii] & core_req_ready_o[iii]) begin 
        idx = iii; 
        break;
      end
    end
