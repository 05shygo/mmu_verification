module mmu_l2tlb_replacement_policy#(
   ...
   // Outputs
    ...
    logic [WAY_NUM-1:0][RRPV_WIDTH-1:0] rrpv_reg;
    logic [WAY_NUM-1:0]                 mask_way_reg;
    logic [WAY_NUM-1:0][RRPV_WIDTH-1:0] rrpv_aged;
    ...
    // SRRIP Logic Signals
    logic [RRPV_MAX:0][WAY_NUM-1:0]     rrpv_sel;   // Bitmap: ways matching specific RRPV
    logic [RRPV_MAX:0][WAY_NUM-1:0]     rrip_repl;  // One-Hot: selected candidate per RRPV level
    ...
    //	    assign mask_rrpv[i] = {RRPV_WIDTH{mask_way[i] & entry_rrpv[i]};		
    //	end
	...
	always_comb begin
		...
		if((entry_rrpv[j] == i) & mask_way[j]) begin
		    rrpv_sel[i][j] = 1'b1;
		end
	    end
	end
	...
	always_comb begin
		...
		if(rrpv_sel[i][k] && (rrip_repl[i] == '0)) begin
		    rrip_repl[i][k] = 1'b1;
		end
	    end
	end
    ...
    always_comb begin
        for (int i = 0; i < WAY_NUM; i++) begin
            ...
            if (entry_rrpv[i] != RRPV_MAX) 
                rrpv_aged[i] = entry_rrpv[i] + 1'b1;
            else
                rrpv_aged[i] = entry_rrpv[i];
        end
    end
        ...
        case ({hit,miss,ptw_req})
            ...
            3'b100: begin
                for (int i = 0; i < WAY_NUM; i++) begin
                    ...
                    rrpv_updata[i] = (hit_index[i] == 1) ? //onehot
                                        {RRPV_WIDTH{1'b0}} : rrpv_reg[i];
                end
            end
            ...
            3'b001: begin
                for (int i = 0; i < WAY_NUM; i++) begin
                    if (victim_way_out[i]) 
                        ...
                        rrpv_updata[i] = rrpv_reg[i]; // Masked/Invalid Ways -> Hold
                end
            end
