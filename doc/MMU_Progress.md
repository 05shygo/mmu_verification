...
| **Phase 11** | v3.0 Gap-driven 回归                       | B 主，A 配合       | ✅ 完成（2026-04-28）                           | ✅`bug_hunt_tests` / `ptw_lsu_protocol_tests` / `mmu_*_list` / `phase11_b_stage_manifest.csv` / bug-hunt matrix 融合结论已冻结；`Makefile` `regress_v3_gap` / `phase11_exit_check` / `phase11_show_failures` 与 `phase11_exit_check.sh` 门禁已闭环；当前项目按 `make phase11_exit_check` 完成记档 |
...
- B 侧：✅ `bug_hunt_tests` 12 个 wrapper + suite、`ptw_lsu_protocol_tests` 5 个 wrapper + suite、`simu/mmu_bug_hunt_list` / `mmu_ptw_lsu_protocol_list` / `mmu_v3_regression_list`、[phase11_b_stage_manifest.csv](phase11_b_stage_manifest.csv) 与 bug-hunt matrix 结论已冻结
...
- Stage split 融合口径：Phase 11 按 “baseline freeze → repo gap audit → 命名/list/blocked 契约 → bug_hunt 真缺陷组 → R19/R20 gate → PTW->LSU protocol → 三份回归列表冻结 → exit gate” 顺序执行。blocked/xfail/DOC_REVIEW 不作为隐式通过，必须在 manifest 中给出 `blocked_reason`、`xfail_required` 与 review mode。
...
| 状态 | `Functional`、`Blocked-Waiting-RTL-Fix`、`DOC_REVIEW`、`xfail_required` 均以 manifest 为准 |
| Gate | `make phase11_exit_check` 统一检查 compile、proof/review 文件、focused seeds、union regression 与 log scan |
| 证据 | 版本库保留脚本、list、manifest 和 review 结论；大体量回归输出由运行目录/服务器归档 |
