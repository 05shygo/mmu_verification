...
> **文档版本**：v3.1（L2TLB RTL 三次核对：dedup 迁移 + RRPV-aging 修正 + 5 条新 BUG_HUNT）
> **发布日期**：2026-04-23
...
| v3.1 | 2026-04-23 | Verification Team | **L2TLB 三次 RTL 核对（以 RTL 为 ground truth）**：(1) §2.2 框图修正 “Miss Buffer 8” → “Miss Buffer 9 (1 ITLB + 8 DTLB)”，与 `mmu_l2tlb_mb.sv` `TOTAL_DEPTH=1+8` 对齐；(2) **F3.7 关键修正**：RTL 实际 hit 行为是 *hit way→0，其余 valid way 同步 +1 老化*（`rrpv_updata[i] = hit_index[i] ? 0 : rrpv_aged[i]`），原描述“其它 ways RRPV 不变”错误，已同步 covergroup/SVA；(3) **F3.13/F3.26 dedup 迁移到 PTW MBUF（方案 A）**：`mmu_l2tlb_mb` 仅做 FFZ 分配，无任何 VPN/ASID 比较——删除 L2TLB 侧 dedup 描述与 TC-MB-DEDUP-001，dedup 验证统一挂到 F4.36 PTW MBUF；(4) **F3.21 标记迁移**：pplru 是 PTW 侧 16-way 伪 LRU，与 L2TLB 替换无关，标 [迁移到 F4 PTW]；(5) F3.12 改为“4 深度 FIFO + youngest-wins CAM bypass，lookup_hit 寄存一拍”；(6) F3.9 victim 描述补“反向优先编码 RRPV_MAX→0 找首个有候选 way，不强制等于 MAX”；(7) F3.38 maee/no_op_req 与 L2TLB clk_en 无直接关系，改为不适用并降级；(8) **新增 5 条 P0/P1 BUG_HUNT (F3.NEW.2~6)**：reqq `ID_W=3` 不足以编码 9 entry（trans_id 截断误命中 entry0）、rrpv_wbuf 默认 `RRPV_WIDTH=2` 与顶层 3 不一致、MB 缺 sfence/abort 通路、`arb_l2tlb_is_dtlb` 表达式重复（store→ITLB 误判）、`raw_vld` 触发条件 `||/!=` 永远为真；(9) §5.3 F3 标题“14 条”→“46 条”；同步更新 `MMU_Traceability_Matrix.csv` / `MMU_GapAudit_v1.md` / `MMU_UVM_BuildPlan_v3_final.md` |
...
```
       ...
       STAMO    ─▶│                 │                │
                   ...
                   │  ReqQ 9 / Miss Buffer 9 (1 ITLB+8 DTLB)  │
                   └──┬───────────────────────┬──────────────┘
                      │ miss                  │ refill to L1
                      ▼                       └──▶ ITLB/DTLB
                   ┌─────────────────────────────────────────┐
...
| 7 | LSU Data（PTW 取 PTE 通道） | OUT/IN | `mmu_lsu_data_req/addr/size`、`lsu_mmu_data/data_vld/bus_error` | PTW 通过 LSU 发起 PTE 读；**v3.0 明确：采用严格串行单 outstanding 握手协议**——`mmu_lsu_data_req` 拉高后必须保持 `mmu_lsu_data_req` 和 `mmu_lsu_data_req_addr` 稳定直到 `lsu_mmu_data_vld` 返回；无 tag/ID 字段，一次最多一个 outstanding，禁止 LSU 乱序返回（F4.42a~c） |
| 7a | LSU 流控（**DTLB→LSU**） | OUT | `mmu_lsu_tlb_busy`、`mmu_lsu_tlb_wakeup[11:0]` | **v3.1 修正**：实际驱动模块为 [`mmu_l1dtlb_install`](../mmu/rtl/mmu_l1dtlb_install.sv)（非 PTW）。`mmu_lsu_tlb_busy` 仅在 `&mb_entry_vld`（DTLB MB 全满）拉起；`mmu_lsu_tlb_wakeup[11:0]` 为 12-bit 同值广播（硬编码 `{12{...}}`，宽度 12 ≠ MB_DEPTH=8），唤醒源为 **`mb_have_free \| l1dtlb_expt_for_taken`**——即 MB 释出空槽 **或** PTW/L2 异常返回；与 PTE 取数握手 (#7) 无关 |
...
| F2.3 | `mmu_l1dtlb` | Miss Buffer 管理：8 深 MB + **5-bit credit（`$clog2(9)+1=5`，CREDIT_MAX=8；模块顶层参数 `CREDIT_WIDTH=3` 为死参数，scheduler 内部自计算位宽，详见 M5）**；MB entry FSM 涵盖 IDLE/WFG/WFC/WFI/PGFLT/ACFLT/ABT **七状态**转换（GAP-D2.1；RTL 确认：WFG=等待 L2 仲裁 grant，ACFLT=访问错误） | [mmu_l1dtlb.sv](mmu/rtl/mmu_l1dtlb.sv#L10), [mmu_l1dtlb_mb_entry.sv#L105-L160](mmu/rtl/mmu_l1dtlb_mb_entry.sv#L105), [mmu_l1dtlb_scheduler.sv#L60-L100](mmu/rtl/mmu_l1dtlb_scheduler.sv#L60) | P0 | DTLB_MB_001, DTLB_MB_002, DTLB_CREDIT_001, DTLB_MB_FSM_WFI_001 | cg_dtlb, sva_credit_conserv |
...
| F2.10 | `mmu_l1dtlb_scheduler` | 信用调度：两路并发公平性；credit 边界（0 / CREDIT_MAX=8）与同周期 req_fire & credit_return 环绕（GAP-D2.4）。**v3.1 修正**：scheduler 内部 `credit_cnt` 实际为 5-bit（`$clog2(9)`+1），与顶层 `CREDIT_WIDTH=3` 参数无关；`bypass_en = ~|mb_entry_ready`，sel_mb 优先级高于 sel_bypass | [mmu_l1dtlb_scheduler.sv#L60-L145](mmu/rtl/mmu_l1dtlb_scheduler.sv#L60) | P0 | DTLB_SCHED_001, DTLB_CREDIT_002, DTLB_CREDIT_BOUND_001, DTLB_SCHED_BYPASS_PRIO_001 | cg_scheduler, sva_fairness, sva_bypass_priority |
...
| F2.17 | `mmu_l1dtlb_install` | **`mmu_lsu_tlb_wakeup[11:0]` 为 DTLB→LSU 广播信号**（实际驱动方为 install 模块，**非 PTW**）：`wakeup_vec_next = {12{mb_have_free}} \| {12{l1dtlb_expt_for_taken}}`，**MB 释出空槽 OR PTW/L2 异常返回** 任一即触发，全 12 位同值广播；**非 per-entry one-hot**；宽度硬编码 12 ≠ `MB_DEPTH=8`，本质等价 1-bit（K5/GAP-D2.15；M2/M6 修正） | [mmu_l1dtlb_install.sv#L233-L290](mmu/rtl/mmu_l1dtlb_install.sv#L233) | P0 | DTLB_WAKEUP_BCAST_001, DTLB_WAKEUP_EXPT_001 | sva_wakeup_broadcast, sva_wakeup_uniform |
...
| F2.20 | `mmu_l1dtlb_mb_entry` | **v3.1 修正**：PGFLT/ACFLT 状态 **仅持续 1 周期**（`state_nxt = STATE_IDLE`），对 LSU 输出表现为 **1-cycle pulse**（无 hold）；`abort_hold_r` 在 IDLE 或 `state_nxt==IDLE` 窗口被清除；ABT 后 refill 晚到防伪 install（GAP-D2.8 / GAP-D2.9 / GAP-D2.13） | [mmu_l1dtlb_mb_entry.sv#L121-L195](mmu/rtl/mmu_l1dtlb_mb_entry.sv#L121) | P1 | DTLB_MB_PGFLT_001, DTLB_MB_ABT_LATE_REFILL_001, DTLB_MB_ABTHOLD_CLR_001 | sva_no_install_after_abt, sva_pgflt_pulse_1cyc, sva_abthold_clr |
...
| F2.A1 | `mmu_l1dtlb_install` | **JTLB hit-refill 路径不会产生 ACFLT**：`req_jtlb_expt = jtlb_dutlb_pgflt`（`jtlb_dutlb_acc_err` 在 RTL 中被注释掉），仅 PGFLT 通路有效；负向覆盖确认 ACFLT 仅来自 PTW（v3.1 新增） | [mmu_l1dtlb_install.sv#L260-L290](mmu/rtl/mmu_l1dtlb_install.sv#L260) | P1 | DTLB_JTLB_NO_ACFLT_001 | sva_jtlb_no_acflt |
| F2.A2 | `ct_mmu_top` + `ptw_mbuf` | **PTW→DTLB ID 截断**：顶层连接 `.ptw_l1dtlb_ref_id (ptw_l1dtlb_id[2:0])`，6→3 bit 截断；需验证截断后 ID 与 DTLB MB 8-entry 索引无 alias、不会误投递（v3.1 新增） | [ct_mmu_top.v#L106](mmu/rtl/ct_mmu_top.v#L106), [ptw_mbuf.sv](mmu/rtl/ptw_mbuf.sv) | P1 | DTLB_PTW_ID_TRUNC_001 | sva_ptw_dtlb_id_no_alias |
| F2.A3 | `mmu_l1dtlb` | **`dutlb_l2tlb_req_is_load` 极性 sanity**：RTL `assign dutlb_l2tlb_req_is_load = ~dutlb_l2tlb_req_store;`（mmu_l1dtlb.sv L747），即 `is_load = !store`——发给下游请求**不是 store 时为 1**；用于跨模块语义对齐确认（v3.1 新增） | [mmu_l1dtlb.sv#L121,L747](mmu/rtl/mmu_l1dtlb.sv#L747) | P2 | DTLB_REQ_ISLOAD_POL_001 | sva_isload_polarity |
| F2.A4 | `mmu_l1dtlb_scheduler` | **bypass 通路使能与优先级**：`bypass_en = ~|mb_entry_ready`（无任何 ready MB entry 时才允许 bypass），`sel_mb` 优先级 > `sel_bypass`；验证 MB 单 ready 时 bypass 不抢占（v3.1 新增） | [mmu_l1dtlb_scheduler.sv#L60-L145](mmu/rtl/mmu_l1dtlb_scheduler.sv#L60) | P1 | DTLB_SCHED_BYPASS_PRIO_001 | sva_bypass_priority |
| F2.A5 | `mmu_l1dtlb_mb_entry` | **`abort_hold_r` 清除窗口**：仅在 `state == IDLE` 或 `state_nxt == IDLE` 时被清；其他 abort 进入但 entry 仍被占用的场景需保持；负向场景：abort 后 PTW 响应在 IDLE 之前到达，install 必须被 `abort_hold_r` 屏蔽（v3.1 新增） | [mmu_l1dtlb_mb_entry.sv#L121-L195](mmu/rtl/mmu_l1dtlb_mb_entry.sv#L121) | P1 | DTLB_MB_ABTHOLD_CLR_001 | sva_abthold_clr |
| F2.A6 | `mmu_l1dtlb_mb_entry` | **WFC + abort + refill 三重并发**：WFC 状态同周期收到 `abort` 与 PTW refill；分两支验证：(1) `issue_grant=1` 路径 → 进 ABT 等迟到响应；(2) `issue_grant=0` 路径 → install 被 `abort_hold_r` 抑制；entry 必须正确回收，不得污染 DTLB 数据阵列（v3.1 新增） | [mmu_l1dtlb_mb_entry.sv#L121-L195](mmu/rtl/mmu_l1dtlb_mb_entry.sv#L121) | P0 | DTLB_MB_WFC_ABT_REFILL_001, DTLB_MB_WFC_ABT_REFILL_002 | cg_mb_fsm_7state, sva_no_install_after_abt |
| F2.A7 | `mmu_l1dtlb` | **【RTL Lint】未定义 `MB_WIDTH` 使用**：`mmu_l1dtlb.sv` ~L244 处 `logic [MB_WIDTH-1:0][PGS_WIDTH-1:0] mb_entry_pgs;` 引用未定义符号 `MB_WIDTH`，疑为 `MB_DEPTH-1` 或 `$clog2(MB_DEPTH)` 笔误；需 RTL fix + lint 守护（v3.1 GAP P1 新增） | [mmu_l1dtlb.sv#L244](mmu/rtl/mmu_l1dtlb.sv#L244) | P1 | DTLB_LINT_001 | sva_lint_clean |
| F2.A8 | `mmu_l1dtlb_install` | **【RTL 死代码】PLRU bank1 路径未连接**：`plru_bank1_refill_way` 在 install 模块未被消费，bank1 PLRU 更新路径疑似缺失或被 bank0 共用；需评估 16-entry PLRU 双 bank 是否对称、是否影响替换公平性（v3.1 GAP P2 新增） | [mmu_l1dtlb_install.sv](mmu/rtl/mmu_l1dtlb_install.sv) | P2 | DTLB_PLRU_BANK1_DEAD_001 | cg_plru_bank_balance |
...
## § 5.3 F3 L2 TLB/JTLB Feature List（46 条）

| F-ID | 模块 | 功能描述 | 依据 | 优先级 | 对应 Test Case | Covergroup / SVA |
|------|------|---------|------|--------|-----------------|-----------------|
| **F3.1** | `mmu_l2tlb_reqq` | ReqQ 分配：L1 ITLB miss → 分配 entry 0；L1 DTLB miss → FFZ 分配 entry 1–8；**TOTAL_DEPTH = 1+DTLB_DEPTH = 1+8 = 9**（RTL 默认 `DTLB_DEPTH=7` 被顶层覆写为 8）。⚠ `ID_W=3` 在顶层未同步覆写，见 F3.NEW.2。 | [mmu_l2tlb_reqq.sv](mmu/rtl/mmu_l2tlb_reqq.sv) L37–60 | P0 | TC-L2TLB-001 ~ 003 | `l2tlb_reqq_cg` |
...
| **F3.3** | `mmu_l2tlb_reqq` | ReqQ credit：上游 L1 在发请求前按 credit 自检（满则不发）；entry dealloc 时返还 i_credit_return / d_credit_return。 | [mmu_l2tlb_reqq.sv](mmu/rtl/mmu_l2tlb_reqq.sv) L100–120 | P0 | TC-L2TLB-007 ~ 009 | `l2tlb_reqq_cg` / `mmu_l2tlb_sva` |
...
| **F3.5** | `mmu_l2tlb` Data 阵列 | Data read：命中时输出 **PPN(28) + FLG(14)**（FLG 完整位域以 RTL 为准，包含 V/R/W/X/U/G/A/D 及其他属性位，不在本表枚举）；支持任意 bank 并行读（8 bank 独立） | [ct_mmu_l2tlb_data_array.sv](mmu/rtl/ct_mmu_l2tlb_data_array.sv) | P0 | TC-L2TLB-013 ~ 015 | `l2tlb_bank_cg` |
| **F3.6** | `mmu_l2tlb_replacement_policy` | RRPV 初值：新 refill entry 的 RRPV = INIT = 4（RRPV_MAX-3 = 7-3 = 4）。⚠ RTL 注释 “e.g., 6” 为旧版残留，以 `RRPV_MAX-3=4` 为准。 | [mmu_l2tlb_replacement_policy.sv](mmu/rtl/mmu_l2tlb_replacement_policy.sv) L14 | P0 | TC-RRPV-001 ~ 003 | `l2tlb_bank_cg` |
| **F3.7** | `mmu_l2tlb_replacement_policy` | **【v3.1 修正】RRPV promote on hit + aggressive aging**：命中时 hit way RRPV → 0，其它所有 valid way 同步 +1 老化（饱和于 MAX=7）。RTL：`rrpv_updata[i] = hit_index[i] ? 0 : rrpv_aged[i]`，其中 `rrpv_aged` 已是 +1 饱和后的结果。原描述“其它 ways RRPV 不变”与 RTL 不符，已修正。 | [mmu_l2tlb_replacement_policy.sv](mmu/rtl/mmu_l2tlb_replacement_policy.sv) L146–155 | P0 | TC-RRPV-004 ~ 006 | `l2tlb_bank_cg` / `mmu_l2tlb_sva` |
...
| **F3.9** | `mmu_l2tlb_replacement_policy` | Victim way 选择：First-Free 优先；无空闲时按 RRPV_MAX→0 **反向优先编码** 找首个有候选 way（不强制等于 MAX，没有“aging-until-MAX exists”循环，非经典 SRRIP 变体） | [mmu_l2tlb_replacement_policy.sv](mmu/rtl/mmu_l2tlb_replacement_policy.sv) L75–110 | P0 | TC-RRPV-010 ~ 012 | `l2tlb_bank_cg` / `mmu_l2tlb_sva` |
...
| **F3.12** | `mmu_l2tlb_rrpv_wbuf` | RRPV 延迟回写：hit/aging 的 RRPV 更新进入 **4 深度 FIFO + youngest-wins CAM bypass**，由 arb 仑裁后写回 SRAM，`lookup_hit` 寄存一拍。不是严格 T+1 单拍延迟。 | [mmu_l2tlb_rrpv_wbuf.sv](mmu/rtl/mmu_l2tlb_rrpv_wbuf.sv) | P1 | TC-RRPV-013 ~ 014 | `l2tlb_bank_cg` |
...
| **F3.26** | `mmu_l2tlb_mb` | **【v3.1 修正·GAP-MB.3 证伪完成】** `mmu_l2tlb_mb` 仅做 FFZ 分配，**无任何 VPN/ASID 比较**；原"同 VPN 不同 hash 槽位 → dedup 逻辑确保单 PTW walk"描述不成立（已证伪）。本 feature 仅保留：FFZ 多 free entry 时偏置行为（GAP-MB.5）。TC-MB-DEDUP-001 已删除，dedup 验证统一归属 **F4.36 PTW MBUF** | [mmu_l2tlb_mb.sv#L80-L140](mmu/rtl/mmu_l2tlb_mb.sv#L80) | P1 | TC-MB-FFZ-BIAS-001 *(TC-MB-DEDUP-001 已删，迁至 F4.36)* | `mbuf_alloc_cg` |
...
|------|------|---------|---------|------|--------|---------|
| F4.1 | PTW | SATP 根地址加载 | 从 `regs_ptw_satp_ppn[27:0]` 读取已选中的 SATP PPN，用于 L0 (顶层) 页表基址；**单 SATP 接口**（双 SATP 选择由 `cp0_mmu_satp_sel` 在 regs/CP0 模块完成，PTW 仅看到选好的值）；`regs_ptw_clr` 触发 PDE cache 复位但 **不直接清 abort_flop** | [ptw.sv#L22](mmu/rtl/ptw.sv#L22) | P0 | ptw.sv:input regs_ptw_satp_ppn |
| F4.2 | PTW/TWU | L0 级（顶级，离 SATP 最近）PTE 读取与 PDE Cache 命中 | `PDE_cache` 检测 L2PDE 命中（`L2PDE_xbar_hit_vld`，对应 vpn[26:9] 18-bit tag），hit 时直接用缓存 PPN 进入下一级；miss 时经 `one_to_four_xbar` 分发到空闲 TWU，TWU 发起 LSU 读 (addr = `{satp_ppn, vpn[26:18], 3'b0}`) | [ptw.sv#L141-L154](mmu/rtl/ptw.sv#L141), [twu.sv#L427](mmu/rtl/twu.sv#L427) | P0 | ptw.sv / L2PDE_cache.sv |
| F4.3 | PTW/TWU | L1 级中间 PDE 读取 | `PDE_cache` 检测 L1PDE 命中（vpn[8:0] 9-bit tag）；miss 时 TWU 发 LSU 读 (addr = `{scd_ppn, vpn[17:9], 3'b0}`)，可能触发 L1PDE refill | [ptw.sv#L141-L154](mmu/rtl/ptw.sv#L141), [twu.sv#L587](mmu/rtl/twu.sv#L587), [L1PDE_cache.sv](mmu/rtl/L1PDE_cache.sv) | P0 | L1PDE_cache.sv:input ptw_vpn |
| F4.4 | PTW/TWU | L2 级（最末，叶 PTE）读取与权限提取 | TWU 读出最后一级 PTE (addr = `{thd_ppn, vpn[8:0], 3'b0}`)；RTL 内部使用 14-bit flag (`fst_chk_flg[8:0]` + sysmap_mmu_flg 等派生位)，**非 RISC-V 标准 8-bit PTE flag**；leaf 判定 `fst_chk_leaf_vld = flg[0] & (flg[1] | flg[3])`（V=1 且 R 或 X 任一为 1） | [twu.sv#L745](mmu/rtl/twu.sv#L745), [twu.sv#L475-L499](mmu/rtl/twu.sv#L475) | P0 | twu.sv:output twu_arb_ref_data_din |
| F4.5 | TWU/MBUF | 4 并发 TWU + 9-entry MBUF + per-TWU 占用跟踪 | `ptw.sv` 实例化 **4 个 TWU** (`twu_one`/`twu_two`/`twu_three`/`twu_four`)；MBUF 实为 **9 entry** (`mbuf_entry_vld[8:0]`)：entry[0..7] 由 FFZ 分配，entry[8] 为 ITLB 专用槽位；`twu_have[3:0]` 跟踪每个 TWU 在 MBUF 中是否还有活跃 entry，供 `one_to_four_xbar` 判定 TWU idle；非 VPN dedup | [ptw.sv#L316,L392,L469,L545](mmu/rtl/ptw.sv#L316), [ptw_mbuf.sv#L95-L96,L168-L185](mmu/rtl/ptw_mbuf.sv#L95) | P0 | ptw.sv:twu instances; ptw_mbuf.sv:twu_have |
| F4.6 | MBUF | MBUF 流控（FFZ + ITLB sel） | 9 entry MBUF；`mbuf_entry_upd[8:0]` 由 FFZ (`casez(mbuf_entry_vld[7:0])`) 选择最低空槽分配；entry[8] 仅当 `twu_itlb_sel` 为真时占用；MBUF 全占（无可用槽）时 `mbuf_entry_upd = 9'b0`，新 TWU req 该 cycle 不被接收（隐式反压，**不直接拉 LSU 侧 busy**） | [ptw_mbuf.sv#L262-L290](mmu/rtl/ptw_mbuf.sv#L262) | P0 | ptw_mbuf.sv:mbuf_entry_upd |
| F4.7 | PDE_cache / L2PDE_cache | L2 PDE Cache 替换与失效 | L2 PDE Cache 由 **16 个 `L2PDE_cache` entry 实例**（`PDE_cache.sv` L169 `for L2PDE_ent = 0..15` generate）组成，每 entry 单独 18-bit tag；由 `pplru` 16-way PLRU 选择替换 victim；`regs_ptw_clr` 一拍内全清所有 entry valid | [L2PDE_cache.sv#L40-L92](mmu/rtl/L2PDE_cache.sv#L40), [PDE_cache.sv#L169-L171](mmu/rtl/PDE_cache.sv#L169) | P1 | L2PDE_cache.sv |
| F4.8 | PDE_cache / L1PDE_cache | L1 PDE Cache 替换与失效 | L1 PDE Cache 由 **16 个 `L1PDE_cache` entry 实例**（`PDE_cache.sv` L146 `for L1PDE_ent = 0..15` generate）组成，每 entry 单独 9-bit tag；由 `pplru` 16-way PLRU 选择 victim；**RTL 中无 TTL 老化逻辑**，仅靠 `regs_ptw_clr` 全清 + 命中替换更新 | [L1PDE_cache.sv#L46-L92](mmu/rtl/L1PDE_cache.sv#L46), [PDE_cache.sv#L146-L148](mmu/rtl/PDE_cache.sv#L146) | P1 | L1PDE_cache.sv |
| F4.9 | TWU | PTE V=0（非法）检测与终止 | TWU 在 `fst_chk_page_flt` 表达式中检查 `!fst_chk_flg[0]`（V=0）触发 page fault，终止 walk | [twu.sv#L485](mmu/rtl/twu.sv#L485) | P0 | twu.sv:fst_chk_flg[0] |
| F4.10 | TWU | PTE 权限组合非法检测 | RTL 实际语义（[twu.sv#L485-L498](mmu/rtl/twu.sv#L485)）：(a) leaf 必须 V=1 且 (R=1 \| X=1)；(b) **W=1 且 R=0** 且 X=0 (write-only) 触发 page fault；(c) load 类型时 R=0 且 (`!cp0_mmu_mxr` 或 X=0) 触发；(d) store 时 W=0；(e) fetch 时 X=0；触发后通过 `fst_chk_page_flt` 上报；区分 access fault 由 PMP/sysmap 路径 (`fst_pmp_deny`) 处理 | [twu.sv#L485-L498](mmu/rtl/twu.sv#L485) | P0 | twu.sv:fst_chk_page_flt |
| F4.11 | TWU | Reserved Bits 非零检测 | Sv39 PTE reserved bits（63:62, 60:55）非零的处理；RTL 实际从 `fst_chk_data` 中提取的 `fst_chk_flg[8:0]` 仅含 (data[9:6] + data[4:0])，**reserved bits 检查未在 fst_chk_page_flt 中显式列出**；需结合 RSW 测试（F4.55）一并覆盖；列为 P1 待审 | [twu.sv#L475-L498](mmu/rtl/twu.sv#L475) | P1 | twu.sv:fst_chk_flg |
| F4.12 | TWU | Misaligned PPN 检测 | RTL 在 `fst_chk_page_flt` 中包含 `fst_chk_data[27:10] != 18'b0` 的 1G 对齐检查（[twu.sv#L496](mmu/rtl/twu.sv#L496)）；2M 对齐检查需在 `scd_chk` 路径独立确认 | [twu.sv#L496](mmu/rtl/twu.sv#L496) | P1 | twu.sv:1g align |
...
| F4.15 | TWU | U bit 与 SUM 交互 | Supervisor 访问 User 页面时（U=1, priv=S）：`fst_chk_flg[4] && fst_chk_cp0_supv_mode && !cp0_mmu_sum` 触发 page fault；SUM=1 时允许 | [twu.sv#L491](mmu/rtl/twu.sv#L491) | P0 | twu.sv:input cp0_mmu_sum |
| F4.16 | TWU | X/W/R 权限检查 | 在 `fst_chk_page_flt` 集中实现：load 检 R=1 或 (MXR 且 X=1)；store 检 W=1；fetch 检 X=1；MXR (`cp0_mmu_mxr`) 允许 X=1 R=0 PTE 被读 | [twu.sv#L486-L491](mmu/rtl/twu.sv#L486) | P0 | twu.sv:input cp0_mmu_mxr |
| F4.17 | TWU | 全局页（G bit）支持 | G bit 来源于 PTE `fst_chk_data[5]`，被打包进 refill tag 末位 (`fst_chk_refill_tag = {1'b1, vpn, asid, 3'b100, fst_chk_data[5]}`) 写入 L2 TLB；L2 TLB 命中比较时根据该位忽略 ASID | [twu.sv#L506](mmu/rtl/twu.sv#L506) | P0 | twu.sv:fst_chk_refill_tag |
| F4.18 | PTW/TWU | 巨页降级（1G→2M→4K）| `csr_refill_pgs[2:0]` 编码: 1G=`3'b100`, 2M=`3'b010`, 4K=`3'b001`；CSR FSM 检测 sysmap 跨界时 `csr_refill_pgs` 由 1G 降级为 2M；2M 跨界再降为 4K（触发 `ptw_chk_cross`） | [twu.sv#L1095-L1115](mmu/rtl/twu.sv#L1095) | P0 | twu.sv:csr_refill_pgs |
| F4.19 | PTW/TWU | 跨级巨页支持 | CSR FSM 状态 `TWU_1G_CRS1/CRS2` / `TWU_2M_CRS1/CRS2` 独立判定每级页大小；非链式约束，1G L1 hit 直接出 1G page，2M 在 L0 检出，4K 完整三级 walk | [twu.sv#L1041-L1085](mmu/rtl/twu.sv#L1041) | P1 | twu.sv:CSR FSM |
| F4.20 | PTW | SATP 写入与 in-flight walk | PTW 仅看到单一 `regs_ptw_satp_ppn[27:0]` 输入（双 SATP 切换在 regs/CP0 完成）；SATP 写后 **不自动失效 in-flight walk**；需软件配合 SFENCE 触发 `regs_ptw_clr` 来清 PDE cache；**注意 `regs_ptw_clr` 不清 abort_flop**（见 F4.47） | [ptw.sv#L22](mmu/rtl/ptw.sv#L22) | P0 | ptw.sv:input regs_ptw_satp_ppn / input regs_ptw_clr |
| F4.21 | PTW | SFENCE 与 Walk 冲突处理 | `tlboper_ptw_abort` 拉高时若有 `mbuf_entry_on_vld`，`abort_flop` 在 ptw_clk 下置 1（非组合）；当 `lsu_mmu_bus_error \| lsu_mmu_data_vld` 返回时清 0；abort 期间 `ptw_jtlb_ready = pde_cache_ready & !abort_flop` 阻断新请求；避免被 abort 的 walk 结果写入 L2 TLB | [ptw.sv#L194,L246-L252](mmu/rtl/ptw.sv#L194), [ptw_mbuf.sv#L247](mmu/rtl/ptw_mbuf.sv#L247) | P0 | ptw.sv:input tlboper_ptw_abort |
| F4.22 | MBUF | 总线错误（bus_error）提前终止 | PTW 数据通道返回 `lsu_mmu_bus_error=1` 时，对应 entry 的 `bus_err_flop` 置 1，依赖此 entry 的 TWU 通过 `mbuf_twu_data_vld` 接到错误；MBUF 汇总后上报 `mbuf_bus_error` + 类型/ID | [ptw_mbuf.sv#L532-L565](mmu/rtl/ptw_mbuf.sv#L532) | P0 | ptw_mbuf.sv:input lsu_mmu_bus_error |
| F4.23 | PTW (交叉引用) | **仅交叉引用**：`mmu_lsu_tlb_wakeup[11:0]` 与 `mmu_lsu_tlb_busy` 均由 **L1 DTLB** 顶层产生，不是 PTW 输出；详见 [F2.17](#) (wakeup 广播) 与 [F2.18](#) (busy = `&mb_entry_vld`)；PTW 仅作为其触发源之一（`l1dtlb_expt_for_taken = ptw_ref_fault \| l2tlb_ref_fault`） | [mmu_l1dtlb_install.sv#L229-L264](mmu/rtl/mmu_l1dtlb_install.sv#L229), [mmu_l1dtlb.sv#L1229](mmu/rtl/mmu_l1dtlb.sv#L1229) | — | 见 F2.17 / F2.18 | — |
| F4.25 | TWU | PMP 拒绝时的 Walk 终止 | `fst_pmp_deny` 在 [twu.sv#L417-L422](mmu/rtl/twu.sv#L417) 根据访问类型与 `pmp_mmu_flg[3:0]` 组合判定；deny 时终止 walk 上报 access fault；无数据写入 L2 TLB | [twu.sv#L417-L422](mmu/rtl/twu.sv#L417) | P0 | twu.sv:input pmp_mmu_flg |
| F4.26 | TWU | SysMap 命中绕过 Walk | `sysmap_mmu_hit` 命中时 PA 由 `mmu_sysmap_pa = twu_sysmap_adder[PPN_WIDTH+11:12]` 提供，**优先级高于 L2 TLB**；CSR FSM 中设专门状态处理 sysmap mismatch 跨界 | [twu.sv#L1118-L1170](mmu/rtl/twu.sv#L1118) | P0 | twu.sv:input sysmap_mmu_hit |
| F4.27 | PTW | 多 SATP 切换压力 | 频繁 SATP 切换（伴随 `regs_ptw_clr` 脉冲）不应导致 walk 死锁；PDE cache 被 `regs_ptw_clr` 清后新 walk 正常启动；**注意**：`regs_ptw_clr` 并不清 `abort_flop`（[ptw.sv#L246-L252](mmu/rtl/ptw.sv#L246) `always_ff` 列表中仅含 `cpurst_b` 与 `lsu_mmu_bus_error \| lsu_mmu_data_vld`），若 abort 期间 SATP 切换，需等待护送响应到达才能释放；交叉 GAP-PX.2 | [ptw.sv#L22,L246-L252](mmu/rtl/ptw.sv#L22) | P1 | ptw.sv:input regs_ptw_clr |
...
| F4.34a | `ptw_mbuf` | **【新增】ITLB sel 下 mbuf_grant 可多位同点**：当 `twu_itlb_sel=1` 时 [ptw_mbuf.sv#L213-L221](mmu/rtl/ptw_mbuf.sv#L213) `case 5'b1_0000:` 分支使 `mbuf_grant = {fth_itlb_sel, thd_itlb_sel, scd_itlb_sel, fst_itlb_sel}`，可能多位=1（多 TWU 同 cycle 抱团发 ITLB 请求，均入 entry[8]）——与 F4.34 onehot 断言存在边界冲突，需为 ITLB sel 路径独立覆盖 | [ptw_mbuf.sv#L213-L221](mmu/rtl/ptw_mbuf.sv#L213) | P1 | TC-PMBUF-ITLB-MULTIGRANT-001 | cg_mbuf_itlb_grant |
...
| F4.36 | `ptw_mbuf` | **per-TWU exclusive ownership**：`twu_have[3:0]` 扫描 9 个 mbuf entry 的 `mbuf_entry_twu_idx[i]` (one-hot 4-bit)，标记每个 TWU 是否在 MBUF 中还有活跃 entry；供 `one_to_four_xbar` 结合 TWU 流水线 idle 一起判定 TWU 是否可接收新 PDE 请求；**非 VPN dedup 机制**（GAP-PM.6 描述需同步修正） | [ptw_mbuf.sv#L168-L185](mmu/rtl/ptw_mbuf.sv#L168) | P1 | TC-PMBUF-TWUHAVE-001 | sva_twu_exclusive |
...
| PTW-030 | (迁至 DTLB 章节) | F2.17 | — | 该测例已并入 DTLB_WAKEUP_BCAST_001；F4.23 仅交叉引用 | — | — | wakeup 广播与 PTW 无直接产生关系 |
| PTW-031 | test_mbuf_full_backpressure | F4.6 | Directed | MBUF 9 entry 全占时 `mbuf_entry_upd=9'b0` 阻塞新 TWU req | ptw_walk_cg | P0 | MBUF 全占反压（非 LSU 侧 busy，与 F2.18 `mmu_lsu_tlb_busy` 独立） |
...
| TC-GAP-DTLB-006 (`DTLB_WAKEUP_BCAST_001`) | F2.17 | Directed | wakeup_broadcast seq | P0 |
...
| TC-GAP-PTW-013 (`TC-WAKEUP-BCAST-001`) | F2.17 (原 F4.23，已迁至 DTLB 章节) | Directed | wakeup_broadcast seq | P0 |
