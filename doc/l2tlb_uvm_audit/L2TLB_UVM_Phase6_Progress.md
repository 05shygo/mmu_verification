...
Phase 6/7 最初只创建并 review 后续实现的规划/进度文档。2026-05-23 已按 BuildPlan 完成 Phase6C scoreboard/helper core implementation、Phase6D SVA/bind implementation 和 Phase6E directed/negative infrastructure implementation。Phase6E timeout/fairness targeted stress 仍失败并保持 open；6F/6G 仍需后续继续实现和关闭，不能误标为完整 coverage closure。

| 项目 | 路径 | 状态 | 说明 |
| --- | --- | --- | --- |
| Phase 6 BuildPlan | `doc/l2tlb_uvm_audit/L2TLB_UVM_Phase6_BuildPlan.md` | 已创建/已中文化 | 后续实现蓝图；已补充 6A~6G 严格退出门禁。 |
| Phase 6/7 Progress | `doc/l2tlb_uvm_audit/L2TLB_UVM_Phase6_Progress.md` | 已创建/已中文化 | 后续实现 tracker；已增加门禁和证据记录格式；Phase 6C/6D 已更新为 implementation complete，Phase6E 已更新为 `Implemented-open`，6F/6G 仍为 `Complete-doc`。 |
...
| Phase 6E progress record | 本文件第 2、4.4、6、7、8、9 节 | Implemented-open | 已实现 6E base/suite/wrappers/run lists、shadow-delta trigger gate、directed/negative/debug evidence；timeout/fairness targeted stress 失败并作为 open issue 跟踪。 |
...
| UVM/RTL/Makefile/testbench 代码 | Phase6C UVM scoreboard/helper；Phase6D SVA/bind；Phase6E directed/negative tests | 已修改 | 已新增 Phase6C `mmu_l2tlb_txn_shadow`；Phase6D 扩展 `mmu_arb_sva`、`credit_sva`、`mmu_l2tlb_rrpv_sva`，新增 `mmu_l2tlb_mb_sva`；Phase6E 新增 `l2tlb_phase6e_*` base/suite/wrappers/run lists 并接入 `test_pkg.sv`；未修改 DUT/RTL。 |
| 后续实现批准 | Phase6C/6D/6E | 已完成 core/infrastructure implementation | Phase6E 存在 open timeout/fairness failure；6F/6G 仍需后续阶段继续实现和关闭。 |
...
| 检查项 | 状态 | Progress 记录 | 说明 |
| --- | --- | --- | --- |
| 6E 子阶段状态 | Implemented-open | 第 2 节 6E row | Phase6E directed/negative infrastructure 已实现；timeout/fairness targeted stress 失败保持 open，不声明完整 coverage closure。 |
...
| sufficiency 结论 | Partially closed / Open follow-up | 第 4.4 节；第 7 节 `L2TLB-P6-ISSUE-008` | 已补 directed P0、negative 分类、RRPV debug 和 strict trigger gate；PTW disabled/fault source-specific harness、bad completion/control-hazard injector、timeout/fairness root cause 仍未关闭。 |
| directed/negative matrix | Implemented-open | 第 4.4 节；第 6 节 Phase 6E implementation row | Reset/ReqQ/PFU/TLBOP directed、negative classification、timeout/fairness、RRPV debug 均有 wrapper/run-list；部分 negative/source-specific 项仍为 deferred/waiver。 |
...
| 6E 实现状态 | Implemented-open | 第 2 节 6E row；第 6 节 Phase 6E implementation row | 已新增 Phase6E base/suite/wrappers/run lists 并接入 `test_pkg.sv`；directed P0、negative 分类和 RRPV debug 有 targeted run evidence；timeout/fairness 失败保持 open。 |
...
| run-list 证据 | Implemented-open | `simu/l2tlb_phase6e_*_list`；第 6 节 evidence | `l2tlb_phase6e_directed_p0_list` 4/4 pass；`l2tlb_phase6e_negative_list` 2/2 pass 但为分类/waiver；`l2tlb_phase6e_debug_rrpv_list` 1/1 pass；`l2tlb_phase6e_timeout_fairness_list` 0/1 pass。 |
| open failure | Open | 第 7 节 `L2TLB-P6-ISSUE-013` | `test_l2tlb_p6e_timeout_fairness_release` seed 64001 触发真实 shadow delta 后失败，`UVM_ERROR=94`、translation SB 52 mismatch；需 root-cause DUT vs UVM scoreboard。 |
...
| `mmu_arb_sva.sv` bind `mmu_arb` | 新增/强化 five-source onehot、PTW/TLBOP/wbuf/prefetch_mask block、fixed-priority eligibility、source payload no-cross、PFU mask set/release 和 cover properties。 | 覆盖 `L2TLB_SVA_005/006/019/020/021` 稳定 arbiter 子项；未命中 block/conflict cover 转 6G targeted closure。 |
...
| `L2TLB_SVA_014` | must | Implemented-scoped | `mmu_l2tlb_rrpv_sva` | final multi-hit、PTW disabled miss、ReqQ feedback | `comp_fast` + four smoke PASS | terminal cover 0 in current smoke | Terminal release assertions strengthened; multi-hit/PTW-disabled directed trigger remains 6E/6G. |
...
| `L2TLB_SVA_022` | debug | Deferred / 6F | future `mmu_l2tlb_rrpv_wbuf`/arb debug checker | wbuf push/pop/full/empty、wbuf full block | Deferred | Missing | 6D covers arbiter wbuf full blocking of new reads; wbuf no-overflow/no-wrong-grant remains Phase6F debug. |
...
| 2026-05-23 | Phase 6E implementation | `make comp_fast`（workdir: `mmu_verification/`）；`python3 scripts/run_test.py --reg-list simu/l2tlb_phase6e_directed_p0_list --mode run_check --seeds 64001 --timeout 8000000 --jobs 1 --uvm-err-only 1 --uvm-config-db-trace 0`；`python3 scripts/run_test.py --reg-list simu/l2tlb_phase6e_negative_list --mode run_check --seeds 64001 --timeout 8000000 --jobs 1 --uvm-err-only 1 --uvm-config-db-trace 0`；`python3 scripts/run_test.py --reg-list simu/l2tlb_phase6e_debug_rrpv_list --mode run_check --seeds 64001 --timeout 10000000 --jobs 1 --uvm-err-only 1 --uvm-config-db-trace 0`；`python3 scripts/run_test.py --reg-list simu/l2tlb_phase6e_timeout_fairness_list --mode run_check --seeds 64001 --timeout 12000000 --jobs 1 --uvm-err-only 1 --uvm-config-db-trace 0` | Implemented-open | `mmu_verification/output/logs/comp_fast.log`；`mmu_verification/output/logs/test_l2tlb_p6e_reset_active_lookup_ptw_tlbop_pfu_64001.log`；`test_l2tlb_p6e_reqq_arb_payload_owner_64001.log`；`test_l2tlb_p6e_pfu_direct_hit_miss_fault_mask_64001.log`；`test_l2tlb_p6e_tlbop_inv_abort_lifecycle_64001.log`；`test_l2tlb_p6e_ptw_disabled_fault_accerr_64001.log`；`test_l2tlb_p6e_negative_ptw_completion_control_64001.log`；`test_l2tlb_p6e_rrpv_debug_pressure_64001.log`；`test_l2tlb_p6e_timeout_fairness_release_64001.log` | 已新增 Phase6E base/suite/wrappers/run lists。Directed P0 4/4 pass，shadow delta 分别为 reset `activity=348`、ReqQ/arb `activity=606`、PFU `activity=140`、TLBOP/INV `activity=9580`，均 `UVM_ERROR=0`。Negative 2/2 pass 仅为 waiver/classification evidence。RRPV debug 1/1 pass，shadow delta `activity=246`，但 exact victim/RRPV/wbuf 保持 future。 | Timeout/fairness list 0/1 pass：`test_l2tlb_p6e_timeout_fairness_release` seed 64001 shadow delta `activity=544`，但 `UVM_ERROR=94`、translation SB `52 mismatch(es)`、L1DTLB spec SB `P6D_MB_CAM_HIT=6`/`P6D_ALLOC_MISS=6`。该失败保持 open，不得用 waiver 或 effective-pass 掩盖。 |
...
| L2TLB-P6-ISSUE-008 | 2026-05-23 | Regression gap | High | Phase 6E / directed and negative tests | 现有 directed/stress/negative-looking wrapper 数量较多，但缺 per-scenario trigger gate、checker/SVA gate、negative 隔离和 targeted run evidence；PFU、PTW disabled/fault/access error、control hazard、timeout/fairness、reset/TLBOP cross 等 P0/P1 不能直接关闭。 | TBD | Partially closed / Open follow-up | Phase6E 已新增 base/suite/wrappers/run lists 和 shadow-delta trigger gate；directed P0、negative 分类、RRPV debug 有 targeted evidence。PTW disabled/fault source-specific harness、bad-completion/control-hazard injector 和 timeout/fairness failure 仍保持 open。 |
...
| L2TLB-P6-ISSUE-013 | 2026-05-23 | Regression gap | High | Phase 6E / `L2TLB_TP_049..050` | `test_l2tlb_p6e_timeout_fairness_release` seed 64001 在真实 Phase6C shadow delta 后失败：`ptw_req=118 ptw_data=118 l2_hit=74 l2_miss=118 pfu=52 payload_ignore=52 activity=544`，但最终 `UVM_ERROR=94`、translation SB 报 52 个 PA/fault mismatch，L1DTLB spec SB 报 `P6D_MB_CAM_HIT=6` 和 `P6D_ALLOC_MISS=6`。 | TBD | Open | 保持 targeted timeout/fairness list failing/open；下一步必须 root-cause DUT vs UVM scoreboard，修复后重跑 seed 64001 和邻近 seed。不得将该项 waiver 或计入 Phase6E pass closure。 |
...
| L2TLB-WAIVE-P6E-001 | `L2TLB_TP_018`, `025..026`, `033`; `L2TLB_SVA_012/014` | Phase6E 未关闭 PTW disabled/fault/access-error source-specific positive closure | 当前复用 `cp0_ptw_disable_seq`、illegal/bus-error PTW memory 与 PTW thrash 会产生既有 translation scoreboard mismatch，不能作为合法 source-specific L2TLB directed pass。 | `test_l2tlb_p6e_ptw_disabled_fault_accerr` 只发 scoped waiver/classification；PFU payload-ignore 正向证据由 `test_l2tlb_p6e_pfu_direct_hit_miss_fault_mask` 提供。 | ITLB/DTLB/PFU owner-specific PTW disabled/fault/access-error 仍未完整验证。 | TBD | Deferred/Open |
...
| Phase 6E 进度已更新到 Progress | Implemented-open | 第 2 节状态矩阵、第 4.4 节 directed/negative baseline、第 6 节 implementation evidence、第 7 节 issue、第 8 节 waiver/deferred 均已记录；声明 Phase6E infrastructure/wrapper/run-list 已实现，但 timeout/fairness failure、negative/source-specific harness 和 RRPV exact model 未关闭。 |
...
| 后续实现批准 | In progress | Phase6C/6D 已完成 core implementation；Phase6E 已完成 directed/negative infrastructure implementation 但存在 open failure；6F/6G 仍需后续阶段继续实现和关闭。 |
