...
| `stage9_wait_for_pde_accerr()` | 使用 probe 检查 PDE direct accerr type/id stable 和 grant；2026-05-18 修正为先按目标 `{type,vpn}` 同步 PTW accept 并捕获 RTL composite `ptw_id`，再从 accept 后窗口检查 matching `PDE_cache_acc_err_vld/grant` 或顶层 `ptw_l2tlb_ref_acc_err` 可见完成，避免把 LSU stimulus id 误当 PTW id；对无并发 direct-accerr 的 mmode lock matrix，可显式打开 `allow_pre_accept_pde_match`，用同 `{type,vpn}` 的 `PDE_cache_acc_err_vld` 反推目标 composite id，覆盖 PDE direct accerr 已发生但 helper 未采到 ready accept 的合法路径。 |
...
| 2026-05-18 | `test_ptw_pde_mmode_lock_matrix_001_606` 在 `stage9_mmode_l2_lock1_type_deny` 报 `target req was not accepted within 512 cycles`，`saw_any_accept=0`。该子场景无并发 pressure，目标是证明 locked R-only L2 PDE 对 STORE 产生 direct accerr。 | UVM test-local helper 同步前提过强，不能据此判 RTL bug。RTL/SVA 的 direct-accerr 主证据是 `PDE_cache_acc_err_vld/type/id` 和 `acc_err_twu_grant[5]`；该路径可能已由 PDE cache 直接形成，而 helper 仍把 `l2tlb_ptw_req && ptw_jtlb_ready` 作为唯一目标同步点。 | `stage9_wait_for_pde_accerr()` 新增默认关闭的 `allow_pre_accept_pde_match`；仅 `stage9_mmode_l2_lock1_type_deny` 打开，用匹配 `{type=STORE,vpn=deny_vpn}` 的 `PDE_cache_acc_err_vld` 反推目标 composite `ptw_id`，继续检查 type/id stable 与 grant。priority test 保持严格 accept-first。 |
