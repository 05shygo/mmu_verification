...
| 2026-05-18 | `test_ptw_pde_mmode_lock_matrix_001_606` 在 `stage9_mmode_l2_lock1_type_deny` 报 `target req was not accepted within 512 cycles`，`saw_any_accept=0`。 | 最新规格修正后，这是 UVM 旧假设，不是 RTL bug：`priv=S/U, MPRV=1, MPP=M` 的 STORE direct-map VA=PA，不会进入 PTW。 | `test_ptw_pde_mmode_lock_matrix_001` 改为 open/unreachable top-source marker；`doc/ptw_rtl_debug.md` 第 17 条反转为规格修正记录；source monitor/ref model 对 data/PFU MPRV=1 MPP=M PTW accept 报 illegal。 |
