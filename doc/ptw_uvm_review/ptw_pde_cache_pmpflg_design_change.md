...
| `PTW-AUD-011` MPRV/MPP effective privilege | `add` | `modify` | 最新规格下 `MPRV=1 && MPP=M` 的 data/PFU direct-map/no PTW source；fetch 不受 MPRV/MPP。PDE cache cached pmpflg 的 M-mode lock/bypass matrix 不能再用该 top-level data/PFU 构造关闭。 |
...
| `PTW-ADD-040` | `test_ptw_pde_l2_pmp_l2_deny_accerr_001` | `PDE-TP-015`、`PTW-FLOW-026` | L2 entry tag match，cached `l1pmpflg` 允许当前 type，但 cached `l2pmpflg` deny。 | 直接 PDE cache access fault；不回退 scd PMP，不发 LSU；释放对应 L2TLB miss buffer。最新规格修正后，旧 `MPRV=1/MPP=M` data construction 不合法，exact L2-only isolation 需 legal stimulus 或 lower-level injection。 |
...
| `PTW-ADD-043` | `test_ptw_pde_mmode_lock_matrix_001` | `PDE-TP-018`、`PTW-ADD-015` | effective M-mode 下，cached `pmpflg[3]` 分别为 0/1，并交叉当前 type bit allow/deny。 | top-level data/PFU `MPRV=1/MPP=M` source path 不存在；该矩阵需要 lower-level PDE-cache stimulus 或 RTL unit evidence，不可用 source directed data/PFU 请求关闭。 |
