...
| Pure M-mode no-translation request enters PTW | Constrained away for normal tests. `MPRV=1 && MPP=M` data/PFU requests are legal and separately covered. |
...
| `PTW-AUD-011` | MPRV/MPP effective privilege applies to load/store/PFU; fetch ignores MPRV. | `PTW-ADD-015/030/036`, `PTW-FLOW-023` | Context sample transaction and source permission/PMP compare; `PTW-SVA-PMP-007`, `PTW-SVA-CTX-006`, `PTW-SVA-REQ-006`. | M-mode/direct-map L1DTLB tests are consumer/system tests, not PTW source closure. | open |
