...
| `test_ptw_p0_pde_mbuf_pmp_matrix` | PDE hit/update representative paths, fst PMP deny, CHK slow/hold, LSU bus error priority, MPRV/MPP=M LOAD/PFU direct-map consumer sanity with no PTW source expected. |
...
| PTW-ADD-015 | closed-consumer-only | `stage6_mprv_mpp_m_load_direct_map_no_ptw` and `stage6_mprv_mpp_m_pfu_direct_map_no_ptw` record direct-map/no PTW source behavior; fetch with MPRV remains real-privilege source behavior. |
...
| PTW-FLOW-023 | closed-consumer-only | LOAD/PFU MPRV=1 MPP=M direct-map VA=PA and no PTW source; fetch ignores MPRV/MPP and follows real privilege. |
