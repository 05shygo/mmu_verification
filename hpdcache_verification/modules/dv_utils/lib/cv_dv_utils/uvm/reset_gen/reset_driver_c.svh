...
function void build_phase( uvm_phase phase );
    ...
    // 1) ???????????? + ???key
    if (!got_vif) begin
      got_vif = uvm_config_db #(
                  virtual xrtl_reset_vif #( p_init_value, p_init_assert, p_post_reset_delay )
                )::get(this, "", get_name(), m_v_reset_vif);
    end
    ...
    if (!got_vif) begin
      got_vif = uvm_config_db #(
                  virtual xrtl_reset_vif #( p_init_value, p_init_assert, p_post_reset_delay )
                )::get(this, "", "reset_vif", m_v_reset_vif);
    end

    // 3) ? null + ??? full_name??????+ ???key
    if (!got_vif) begin
      got_vif = uvm_config_db #(
                  virtual xrtl_reset_vif #( p_init_value, p_init_assert, p_post_reset_delay )
                )::get(null, get_full_name(), get_name(), m_v_reset_vif);
    end

    // 4) ? null + ??? full_name??????+ reset_vif
    if (!got_vif) begin
      got_vif = uvm_config_db #(
                  virtual xrtl_reset_vif #( p_init_value, p_init_assert, p_post_reset_delay )
                )::get(null, get_full_name(), "reset_vif", m_v_reset_vif);
    end
    ...
    if (!got_vif) begin
      got_vif = uvm_config_db #(
                  virtual xrtl_reset_vif #( p_init_value, p_init_assert, p_post_reset_delay )
                )::get(null, "uvm_test_top.env.hpdcache_reset_driver", "reset_vif", m_v_reset_vif);
    end
    ...
    if (!got_vif) begin
      got_vif = uvm_config_db #(
                  virtual xrtl_reset_vif #( p_init_value, p_init_assert, p_post_reset_delay )
                )::get(null, "uvm_test_top.env.hpdcache_reset_driver", "hpdcache_reset_driver", m_v_reset_vif);
    end

    if (!got_vif) begin
      `uvm_fatal("BUILD_PHASE",
                 $sformatf("Unable to get reset_vif for %s (full=%s) from configuration database",
                           get_name(), get_full_name()))
    end
    else begin
      `uvm_info("BUILD_PHASE",
                $sformatf("Got reset_vif for %s (full=%s)",
                          get_name(), get_full_name()),
                UVM_LOW)
    end
endfunction : build_phase
