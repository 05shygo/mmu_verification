...
module top;
	...
	localparam int MEM_W_ADDR = HPDCACHE_PA_WIDTH;
	localparam int MEM_W_DATA = HPDCACHE_MEM_DATA_WIDTH;
	localparam int MEM_W_ID   = HPDCACHE_MEM_ID_WIDTH;
    ...
    xrtl_reset_vif #(1'b1,32'd50,32'd0) reset_if (.clk(clk),
                                          .reset(reset),
                                          .reset_n(rst_n),
   ...
   // -----------------------------------------------------------------
   memory_response_if#(
       .addr_width (MEM_W_ADDR),
       .data_width (MEM_W_DATA),
       .id_width   (MEM_W_ID)
...
//    assign mem_req_write_base_id_i = HPDCACHE_WRITE_BASE_ID;
   ...
   // assign axi2mem_if.ar_atop   = mem_req_read_o.mem_req_atomic;
    ...
    assign mem_resp_read_i.mem_resp_r_error   = (axi2mem_if.r_resp == RESP_OKAY || axi2mem_if.r_resp == RESP_EXOKAY) ?
    ...
    // ATOMIC signal
    assign  mem_resp_write_i.mem_resp_w_error = (axi2mem_if.b_resp == RESP_OKAY || axi2mem_if.b_resp == RESP_EXOKAY) ?
                                                HPDCACHE_MEM_RESP_OK : HPDCACHE_MEM_RESP_NOK;
   ...
   for (genvar k = 0; k < m_hpdcache_cfg.u.nRequesters; k++) begin : HPDCACHE_REQ
     initial begin
       uvm_config_db #(virtual hpdcache_if)::set(
         null, "*", $sformatf("hpdcache_agent_%0d", k), hpdcache_vif[k]
       );
       uvm_config_db #(virtual hpdcache_if)::set(
         uvm_root::get(), "*", $sformatf("hpdcache_agent_%0d", k), hpdcache_vif[k]
       );
     end
   end

   initial begin
     virtual xrtl_reset_vif #(1'b1,32'd50,32'd0) reset_vif_chk;
     virtual xrtl_clock_vif               clock_vif_chk;
     virtual pulse_if                     flush_vif_chk;
     ...
     uvm_config_db #(virtual dram_if)::set(uvm_root::get(),"*", "DRAM_IF",       dram_vif);

     uvm_config_db #(virtual perf_if)::set(null,           "*", "PERF_IF",       perf_vif);
     uvm_config_db #(virtual perf_if)::set(uvm_root::get(),"*", "PERF_IF",       perf_vif);

     uvm_config_db #(virtual conf_if)::set(null,           "*", "CONF_IF",       conf_vif);
     uvm_config_db #(virtual conf_if)::set(uvm_root::get(),"*", "CONF_IF",       conf_vif);

     uvm_config_db #(virtual misc_if)::set(null,           "*", "MISC_IF",       hpdcache_misc_if);
     ...
     uvm_config_db #(virtual hwpf_stride_cfg_if)::set(uvm_root::get(),"*", "PREFETCHER_IF", hwpf_stride_cfg_vif);

     // -------------------------------------------------------------
     // Reset / Flush / Clock (multi-key + multi-scope robust setup)
     // -------------------------------------------------------------
     // reset_driver_c.svh uses:
     //   uvm_config_db#(virtual xrtl_reset_vif#(...))::get(this, "", get_name(), m_v_reset_vif)
     // i.e. field name usually == "hpdcache_reset_driver"
     // Some envs/tools may also use "reset_vif"
     //
     // 1) wildcard + field=component name
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,32'd50,32'd0))::set(null,            "*", "hpdcache_reset_driver", reset_if);
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,32'd50,32'd0))::set(uvm_root::get(), "*", "hpdcache_reset_driver", reset_if);
/*
     // 2) wildcard + generic alias
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,50,0))::set(null,            "*", "reset_vif", reset_if);
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,50,0))::set(uvm_root::get(), "*", "reset_vif", reset_if);

     // 3) explicit path + field=component name
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,50,0))::set(
       null, "uvm_test_top.env.hpdcache_reset_driver", "hpdcache_reset_driver", reset_if
     );
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,50,0))::set(
       uvm_root::get(), "uvm_test_top.env.hpdcache_reset_driver", "hpdcache_reset_driver", reset_if
     );

     // 4) explicit path + alias field
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,50,0))::set(
       null, "uvm_test_top.env.hpdcache_reset_driver", "reset_vif", reset_if
     );
     uvm_config_db #(virtual xrtl_reset_vif #(1'b1,50,0))::set(
       uvm_root::get(), "uvm_test_top.env.hpdcache_reset_driver", "reset_vif", reset_if
     );
*/
     // Flush driver (pulse_gen)
     uvm_config_db #(virtual pulse_if)::set(null,            "*", "hpdcache_flush_driver", flush_vif);
     uvm_config_db #(virtual pulse_if)::set(uvm_root::get(), "*", "hpdcache_flush_driver", flush_vif);
     uvm_config_db #(virtual pulse_if)::set(null,            "*", "pulse_vif",             flush_vif);
     uvm_config_db #(virtual pulse_if)::set(uvm_root::get(), "*", "pulse_vif",             flush_vif);

     uvm_config_db #(virtual pulse_if)::set(
       null, "uvm_test_top.env.hpdcache_flush_driver", "hpdcache_flush_driver", flush_vif
     );
     uvm_config_db #(virtual pulse_if)::set(
       uvm_root::get(), "uvm_test_top.env.hpdcache_flush_driver", "hpdcache_flush_driver", flush_vif
     );
     uvm_config_db #(virtual pulse_if)::set(
       null, "uvm_test_top.env.hpdcache_flush_driver", "pulse_vif", flush_vif
     );
     uvm_config_db #(virtual pulse_if)::set(
       uvm_root::get(), "uvm_test_top.env.hpdcache_flush_driver", "pulse_vif", flush_vif
     );

     // Clock driver (clock_gen)
     uvm_config_db #(virtual xrtl_clock_vif)::set(null,            "*", "clock_driver", clock_if);
     uvm_config_db #(virtual xrtl_clock_vif)::set(uvm_root::get(), "*", "clock_driver", clock_if);
     uvm_config_db #(virtual xrtl_clock_vif)::set(null,            "*", "clock_vif",    clock_if);
     uvm_config_db #(virtual xrtl_clock_vif)::set(uvm_root::get(), "*", "clock_vif",    clock_if);

     uvm_config_db #(virtual xrtl_clock_vif)::set(
       null, "uvm_test_top.env.clock_driver", "clock_driver", clock_if
     );
     uvm_config_db #(virtual xrtl_clock_vif)::set(
       uvm_root::get(), "uvm_test_top.env.clock_driver", "clock_driver", clock_if
     );
     uvm_config_db #(virtual xrtl_clock_vif)::set(
       null, "uvm_test_top.env.clock_driver", "clock_vif", clock_if
     );
     uvm_config_db #(virtual xrtl_clock_vif)::set(
       uvm_root::get(), "uvm_test_top.env.clock_driver", "clock_vif", clock_if
     );

     // -------------------------------------------------------------
     // Back-pressure agent vifs
     // -------------------------------------------------------------
     uvm_config_db #(virtual bp_vif #(1))::set(null,            "*", "bp_read_agent",       m_read_bp_vif);
     uvm_config_db #(virtual bp_vif #(1))::set(uvm_root::get(), "*", "bp_read_agent",       m_read_bp_vif);

     uvm_config_db #(virtual bp_vif #(1))::set(null,            "*", "bp_write_req_agent",  m_write_req_bp_vif);
     uvm_config_db #(virtual bp_vif #(1))::set(uvm_root::get(), "*", "bp_write_req_agent",  m_write_req_bp_vif);

     uvm_config_db #(virtual bp_vif #(1))::set(null,            "*", "bp_write_data_agent", m_write_data_bp_vif);
     uvm_config_db #(virtual bp_vif #(1))::set(uvm_root::get(), "*", "bp_write_data_agent", m_write_data_bp_vif);

     // -------------------------------------------------------------
     // Memory response model / axi2mem virtual interfaces
     // -------------------------------------------------------------
     uvm_config_db #(virtual memory_response_if#(
       MEM_W_ADDR,
       MEM_W_DATA,
       MEM_W_ID
     ))::set(null,            "*", "mem_rsp_model", mem_rsp_if);
     ...
     uvm_config_db #(virtual axi_if#(
       ...
       1
     ))::set(null,            "*", "axi2mem_req", axi2mem_if);
     ...
     uvm_config_db #(virtual memory_response_if#(
       ...
       MEM_W_DATA,
       MEM_W_ID
     ))::set(null,            "*", "axi2mem_req_rd", mem_rd_if);
     ...
     uvm_config_db #(virtual memory_response_if#(
       ...
       MEM_W_DATA,
       MEM_W_ID
     ))::set(null,            "*", "axi2mem_req_wr", mem_wr_if);
     ...
     // -------------------------------------------------------------
     // DEBUG self-check before run_test()
     // -------------------------------------------------------------
     if (uvm_config_db #(virtual xrtl_reset_vif #(1'b1,32'd50,32'd0))::get(
           null, "uvm_test_top.env.hpdcache_reset_driver", "hpdcache_reset_driver", reset_vif_chk))
       $display("[TOP][CFG][OK] reset via key=hpdcache_reset_driver path=uvm_test_top.env.hpdcache_reset_driver @ %0t", $time);
     else
       $display("[TOP][CFG][NG] reset key=hpdcache_reset_driver path check failed @ %0t", $time);

     if (uvm_config_db #(virtual xrtl_reset_vif #(1'b1,32'd50,32'd0))::get(
           null, "uvm_test_top.env.hpdcache_reset_driver", "reset_vif", reset_vif_chk))
       $display("[TOP][CFG][OK] reset via key=reset_vif path=uvm_test_top.env.hpdcache_reset_driver @ %0t", $time);
     else
       $display("[TOP][CFG][NG] reset key=reset_vif path check failed @ %0t", $time);

     if (uvm_config_db #(virtual xrtl_reset_vif #(1'b1,32'd50,32'd0))::get(
           uvm_root::get(), "uvm_test_top.env.hpdcache_reset_driver", "hpdcache_reset_driver", reset_vif_chk))
       $display("[TOP][CFG][OK] reset via key=hpdcache_reset_driver(root) @ %0t", $time);
     else
       $display("[TOP][CFG][NG] reset key=hpdcache_reset_driver(root) failed @ %0t", $time);

     if (uvm_config_db #(virtual xrtl_clock_vif)::get(
           null, "uvm_test_top.env.clock_driver", "clock_driver", clock_vif_chk))
       $display("[TOP][CFG][OK] clock cfg visible @ %0t", $time);
     else
       $display("[TOP][CFG][NG] clock cfg path check failed @ %0t", $time);

     if (uvm_config_db #(virtual pulse_if)::get(
           null, "uvm_test_top.env.hpdcache_flush_driver", "hpdcache_flush_driver", flush_vif_chk))
       $display("[TOP][CFG][OK] flush cfg visible @ %0t", $time);
     else
       $display("[TOP][CFG][NG] flush cfg path check failed @ %0t", $time);

     $display("[TOP][CFG] All config_db::set done, calling run_test() @ %0t", $time);
