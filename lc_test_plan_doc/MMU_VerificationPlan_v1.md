...
| F4.NEW.1 | `ptw_mbuf` / `PDE_cache` | **PDE Cache 仅非叶 PTE 更新**：`mbuf_cache_upd` 条件 = V=1 & R=0 & X=0（非叶节点）；叶 PTE 不触发 PDE Cache 更新；叶 PTE 命中时验证 Cache 无污染（GAP-PDE.NEW.1） | [ptw_mbuf.sv](mmu/rtl/ptw_mbuf.sv), [PDE_cache.sv](mmu/rtl/PDE_cache.sv) | P0 | TC-PDECACHE-LEAF-001 | cg_pde_leaf_nonleaf, sva_pde_nonleaf_upd |
...
| F4.NEW.3 | `twu` | **thd_chk 路径（4K 页）A bit 检测**：`thd_chk_page_flt` 中 A bit 检测（`!flg[5]`）疑似缺失 — 4K 页 A=0 可能不触发 page fault（需设计确认；GAP-AD.NEW.1/K10 扩展）| [twu.sv](mmu/rtl/twu.sv) | P0 | TC-BUG-002 | cg_twu_stage_fetch, sva_thd_a_bit_pgflt |
...
| F5.NEW.1 | `mmu_arb` | **mask_bank_sel 编码正确性验证**：各 selector 值（VPN[19:18]=00/01/10/11）对应的 8-bit bank mask 字面量编码正确性（疑似无 `8'b` 前缀导致 decimal 字面量 Bug，GAP-Arb.NEW.1）（需设计确认） | [mmu_arb.sv](mmu/rtl/mmu_arb.sv) | P0 | TC-BUG-004 | cg_bank_mask_sel, sva_bank_sel_valid |
...
| TC-BUG-001 | F4.NEW.2 | BUG_HUNT | fst=fetch scd=store 跨级权限场景；验证 scd_chk_page_flt 使用正确 fetch_type | P0 |
| TC-BUG-002 | F4.NEW.3 | BUG_HUNT | 4K PTE A=0 thd_chk_page_flt 验证；期望触发 page fault | P0 |
| TC-BUG-003 | F4.NEW.1 | BUG_HUNT | MAEE=0 场景 thd_chk_refill_req 不应触发 refill；验证 mbuf_cache_upd 仅非叶 PTE | P0 |
| TC-BUG-004 | F5.NEW.1 | BUG_HUNT | selector=00/01/10/11 时 bank mask 字面量（8-bit）编码正确性；直接探针验证 | P0 |
| TC-BUG-005 | F3.4 | BUG_HUNT | PTW 写时 raw_vld OR/AND 逻辑验证；期望 tag compare 在 PTW 写同周期不触发 hit | P1 |
| TC-BUG-006 | F3.5 | BUG_HUNT | store 请求 arb_l2tlb_is_dtlb 标志正确性验证；`3'b110` 类型应被识别为 DTLB | P1 |
| TC-BUG-007 | F3.NEW.1 | BUG_HUNT | SFENCE 无效后 victim 选择受 RRPV 残留影响验证；连续 INVVA + 新分配后替换路 | P1 |
| TC-BUG-008 | F12.NEW.1 | BUG_HUNT | entry 0 多次命中后 PLRU victim 验证；期望复位后第一次命中不造成 tree update 异常 | P1 |
| TC-BUG-009 | F4.NEW.2 | BUG_HUNT | CSR FSM scd_csr_sel=1 场景 csr_grant 分发正确性；验证 `3'b010` case 不重复 | P1 |
| TC-BUG-010 | F4.NEW.2 | BUG_HUNT | 无 csr_req 周期 ptw_nxt_st 无 X 输出；IDLE 无 else 分支 latch 推断验证 | P2 |
...
> 共追加 **72 条** TC-GAP-*/TC-BUG-* 测试用例（原 60 条 + 本次 12 条 BUG_HUNT），分布：P0=24+5、P1=30+5、P2=6+2。所有用例**仅在本表声明**；具体 stimuli/checker 实现交由后续 sequence 开发阶段跟踪 JIRA。
...
| **R15** | **twu 权限模式多级传播 fetch_type 使用错误**（TC-BUG-001/002/003）| **高** | 设计 review 确认 scd/thd_chk_fetch_type 来源；directed TC 精确验证 L0/L1/L2 各级权限语义；`cg_twu_stage_fetch` + `sva_thd_a_bit_pgflt` 锁定 |
| **R16** | **mmu_arb bank mask 编码错误**（TC-BUG-004/F5.NEW.1）| **高** | SVA probe `mask_bank_sel` + directed skew distribution TC；设计 review 确认 literal 编码格式（是否缺少 `8'b` 前缀） |
