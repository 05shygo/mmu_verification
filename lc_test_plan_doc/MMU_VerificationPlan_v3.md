...
| 15 | L1DTLB 状态广播 | OUT | `dutlb_xx_mmu_off` | **v4.0 新增**：L1DTLB 向下游广播 MMU 已关闭状态：`regs_mmu_en=0` 或处于 Machine 模式直接物理地址访问时拉高，通知 L2TLB 等下游模块 MMU 已关闭，翻译请求不会到来（详见 F2.NEW.3） |
...
| F2.17 | `mmu_l1dtlb_install` | **`mmu_lsu_tlb_wakeup[11:0]` 为广播信号**（`mb_have_free=1` 时全 1），**非 per-entry one-hot**（K5/GAP-D2.15）；**[v4.0 规格与 RTL 不一致，需设计确认]**：L1DTLB_Interfaces.md §2.6.3 描述为“12 位独热编码，每 bit 对应一个 LSU 操作项，回填完成时拉高对应 bit”；而 RTL mmu_l1dtlb_install.sv#L233-L235 表现为广播信号（mb_have_free=1 时全 1）。两种行为语义不同，须由设计方澄清最终实现与规格是否一致 | [mmu_l1dtlb_install.sv#L233-L235](mmu/rtl/mmu_l1dtlb_install.sv#L233) | P0 | DTLB_WAKEUP_BCAST_001 | sva_wakeup_broadcast |
...
| F4.24 | PTW / L1DTLB | **tlb_busy 来源澄清（v3.0 修订：源为 L1 DTLB MB 全满，非 PTW MBUF 满）** | `mmu_lsu_tlb_busy=1` 实际由 **L1 DTLB MB 全满**（`&mb_entry_vld`）触发（见 F2.18、mmu_l1dtlb.sv#L1229），**并非 PTW MBUF 满**；PTW MBUF 与 L2TLB MB 配额一一对应不溢出（F4.5/F4.6 已澄清），因此**不需要也不存在 PTW MBUF→tlb_busy 路径**；busy 解除后 LSU 侧停顿立即清除 | [mmu_l1dtlb.sv#L1229](mmu/rtl/mmu_l1dtlb.sv#L1229), [ptw.sv](mmu/rtl/ptw.sv) | P0 | mmu_l1dtlb.sv:assign mmu_lsu_tlb_busy = &mb_entry_vld |
...
| F4.52 | `ptw` + xbar | **twu_mask 语义与传递时延（v3.0 修订：per-TWU 自阻塞，非 MBUF 满反压）** | 每 TWU 的 `twu_mask` 拉高表示**该 TWU 因 PMP/PTE 检查处于 wait、流水中间级暂未让出入口**，当前周期不能接收来自 xbar 的新 PDE 请求；xbar 侧 `twu_ready = ~(&twu_mask[3:0])`（L77）；需验证：单 TWU mask → xbar 对其停止派发且其余 TWU 仍可被命中、四路同时 mask → xbar 全停一周期不丢请求、mask 解除 → 轮转指针正确恢复；**【v5.0 接口补充】**：（1）接口规格文档（MMU_PTW_Interface.md）中 `twu_mask` 对外端口名为 `twu_xbar_mask`，RTL 内部 wire 为 `twu_mask`；（2）当 4 个 TWU 全部 `twu_xbar_mask=1` 时，PTW 对 L2TLB 反压的 `ptw_l2tlb_ready` 信号拉低，停止接受新的 L2TLB miss 请求（见 F4.NEW.6）；（3）四路全停等待时 `twu_req_point_r` 指针**不复位**，防冒冷启动偏向 TWU0（见 F5.NEW.3）；（4）`twu_idle`（完全空闲）与 `!twu_xbar_mask`（未阻塞）语义不等价（见 F4.NEW.7）。 | [twu.sv#L359-L364](mmu/rtl/twu.sv#L359), [one_to_four_xbar.sv#L77,L112](mmu/rtl/one_to_four_xbar.sv#L77), [ptw.sv#L242-L244](mmu/rtl/ptw.sv#L242) | P0 | TC-XBAR-MASK-LAT-001, TC-TWU-MASK-SELF-001, TC-TWU-MASK-ALL-001 | cg_xbar_mask, cg_twu_mask_per_twu, sva_twu_mask_semantics |
| F4.53 | `ptw` / `mmu_l1dtlb` | **mmu_lsu_tlb_busy 语义澄清（v3.0：L1 DTLB MB 全满触发，非 PTW MBUF 满）+ walk watchdog 缺失死锁风险**（GAP-PX.11 / GAP-PX.14） | `mmu_lsu_tlb_busy = &mb_entry_vld` 来自 L1 DTLB（F2.18），与 PTW MBUF 状态**无直接耦合**；验证时需防 stub 把它错配为 PTW MBUF 满 → busy 的映射 | [mmu_l1dtlb.sv#L1229](mmu/rtl/mmu_l1dtlb.sv#L1229), [ptw.sv](mmu/rtl/ptw.sv) | P1 | TC-PTW-BUSY-CONSIST-001, TC-PTW-WATCHDOG-001, TC-BUSY-SRC-DTLB-MB-001 | sva_busy_from_dtlb_mb_only |
...
| F7.NEW.3 | `twu` | **PMP 检查是 PTW 发 LSU 请求的前提——顺序约束（v6.0 新增：用户补充说明 + RTL twu.sv 精读）** | PTW 每次向 LSU 发起 PTE 数据读请求（`mmu_lsu_data_req=1`）之前，必须已完成对应级别的 PMP 权限检查。机制：当 TWU 在某级（FST/SCD/THD）发现叶 PTE 且需要继续 walk 时，先通过 `pmp_grant[2:0]` 发出 PMP 请求，在 `fst/scd/thd_pmp_wait` 阻塞对应流水级期间（等待 PMP 模块返回 `pmp_mmu_flg`），**任何新的 `mmu_lsu_data_req` 不得拉高**；PMP 检查通过（`!pmp_deny`）且对应级别完成后，才允许推进流水线并最终发出 LSU 读请求。若 PMP deny：直接报 Access Fault，不产生 LSU 请求（见 F7.NEW.5）。此顺序约束通过 `twu_mask` 信号（包含 `pmp_wait` 分量，见 F7.NEW.6）体现，防止 xbar 向阻塞中的 TWU 派发新请求，间接阻止提前发出 LSU 访问。验证关注点：（1）在 `pmp_wait=1` 的任意周期，`mmu_lsu_data_req` 必须为 0（SVA 断言）；（2）PMP 返回后的下一周期，LSU 请求可合法拉高；（3）PMP deny 场景下，整个 TWU 流水线不产生 LSU 请求，并在下一可能的机会报 Access Fault（交叉 F4.NEW.13/F7.NEW.5）。 | [twu.sv#L243-246](mmu/rtl/twu.sv#L243) | P0 | TC-PTW-PMP-BEFORE-LSU-001, TC-PTW-PMP-DENY-STOP-REQ-001, TC-PTW-PMP-WAIT-NO-LSU-001 | sva_pmp_check_before_lsu_req, sva_no_lsu_req_during_pmp_wait |
| F7.NEW.4 | `twu` | **PTW 各级 PMP 检查的 PA 格式——页级对齐（v6.0 新增：RTL twu.sv 精读）** | TWU 每个流水级向 PMP 发送的物理地址 `mmu_pmp_pa[27:0]` 格式由 `pmp_grant[2:0]` 多路选择（twu.sv:L1001-1014）：**FST（1G 叶，grant=3'b100）**：PA = `{satp_ppn[PPN_WIDTH-1:0], fst_pmp_vpn[26:18], 3'b0}`（28-bit，1G 页粒度对齐，低 18 bit 为 0）；**SCD（2M 叶，grant=3'b010）**：PA = `{scd_pmp_ppn[PPN_WIDTH-1:0], scd_pmp_vpn[17:9], 3'b0}`（28-bit，2M 对齐，低 9 bit 为 0）；**THD（4K 叶，grant=3'b001）**：PA = `{thd_pmp_ppn[PPN_WIDTH-1:0], thd_pmp_vpn[8:0], 3'b0}`（28-bit，4K 对齐，全部 28 bit 有效）；**default（grant=3'b000）**：PA = 全零。PMP 据此判断对应物理地址的访问权限（R/W/X/L），反馈 `pmp_mmu_flg[3:0]`。PA 对齐格式与 sysmap PA 一致（交叉 F6.NEW.5）。验证关注点：（1）grant=3'b100 时 PA 低 18 bit 为 0；（2）grant=3'b010 时 PA 低 9 bit 为 0；（3）grant=3'b001 时 PA 全 28 bit 有效；（4）PA 与 `mmu_lsu_data_req_addr[39:0]` 高 28 bit 一致性（交叉 F7.4/F7.10）；（5）PMP 模块在同一周期仅接受一路 PA（per-TWU 单端口，交叉 F4.NEW.14）。 | [twu.sv#L1001-1014](mmu/rtl/twu.sv#L1001) | P0 | TC-PTW-PMP-PA-1G-001, TC-PTW-PMP-PA-2M-001, TC-PTW-PMP-PA-4K-001, TC-PTW-PMP-PA-ZERO-001 | sva_pmp_grant_onehot, cg_pmp_pa_format |
| F7.NEW.5 | `twu` | **PMP deny → TWU Access Fault → L2TLB 异常直通（v6.0 新增：用户补充说明 + RTL twu.sv 精读）** | PMP 返回 `pmp_mmu_flg[3:0]` 后，TWU 根据访问类型检查权限（twu.sv:L372-376）：`pmp_deny = (fetch && !flg[2]) || (load && !flg[0]) || (store && !flg[1]) || (pref && !flg[0])`；M-mode 特殊规则：处于 M-mode 且 L-bit（flg[3]）=0 时 PMP 检查被绕过（Sv39 规范）。PMP deny 时：（1）TWU 不发出 LSU 请求、不进行回填；（2）记录 Access Fault：`twu_acc_err_vld <= 1'b1`（twu.sv:L961-964）；（3）通过 `twu_jtlb_ref_acc_err` + `twu_jtlb_ref_acc_err_id[6:0]` + `twu_jtlb_ref_acc_err_type[2:0]` **直接发往 L2TLB，绕过 mmu_arb**（见 F4.NEW.9）。**PTW 访问页表为数据 Load**，使用 R-bit（flg[0]）检查，若 R=0 则产生 Access Fault（R=0 即不可读的内存区域）。验证关注点：（1）PTW 访问 R=0 的 PMP region → Access Fault，不产生 LSU 请求（交叉 F7.NEW.3）；（2）M-mode + L=0 → PMP 检查被绕过，不产生 Access Fault；（3）M-mode + L=1 → PMP 检查生效；（4）Access Fault 后对应 miss entry 正确释放（与 F4.48/F4.NEW.9 交叉）；（5）Access Fault 与 Page Fault 互斥（同一次遍历只报一种异常，与 F4.NEW.9 交叉）。 | [twu.sv#L372-376,L961-964](mmu/rtl/twu.sv#L372), [mmu_l2tlb.sv](mmu/rtl/mmu_l2tlb.sv) | P0 | TC-PTW-PMP-DENY-ACCFLT-001, TC-PTW-PMP-DENY-NO-REFILL-001, TC-PTW-PMP-MMODE-L0-001, TC-PTW-PMP-MMODE-L1-001 | sva_pmp_deny_acc_fault, sva_pmp_deny_no_lsu_req, cg_pmp_deny_by_level |
| F7.NEW.6 | `twu` | **PMP wait 状态触发 `twu_xbar_mask`（v6.0 新增：RTL twu.sv 精读）** | `twu_mask`（接口规格对外名 `twu_xbar_mask`）的生成逻辑包含所有 PMP 等待状态分量（twu.sv:L243-246）：`twu_mask = fst_pmp_wait | scd_pmp_wait | thd_pmp_wait | (fst_chk_vld & !fst_leaf & !scd_pmp_wait) | (scd_chk_vld & !scd_leaf & !thd_pmp_wait)`。其中 `fst/scd/thd_pmp_wait` 在对应级别 PMP 检查请求已发出但尚未获得 grant 时为高。**传播链**：PMP wait → `twu_mask=1` → `ptw_l2tlb_ready` 在全部 4 TWU mask 时拉低（见 F4.NEW.6）→ L2TLB 停止发送新 PTW 请求。**与 twu_idle 的区别**：`twu_mask` 反映"正在 PMP 检查中"，而 `twu_idle` 反映"完全空闲"；`twu_idle=1` 时 `twu_mask=0`，但 `twu_mask=0` 不蕴含 `twu_idle=1`（TWU 可有在途 MBUF 请求但未处于 PMP wait，见 F4.NEW.7）。验证关注点：（1）PMP wait 期间 `twu_mask` 必须为 1；（2）PMP grant 返回后的下一周期 `twu_mask` 应可降低（若无其他阻塞）；（3）4 TWU 全 PMP wait 时 `ptw_l2tlb_ready=0`（压力场景，交叉 F4.NEW.6）。 | [twu.sv#L243-246](mmu/rtl/twu.sv#L243), [ptw.sv](mmu/rtl/ptw.sv) | P1 | TC-TWU-MASK-PMP-WAIT-001, TC-TWU-MASK-ALL4-PMP-001 | sva_pmp_wait_implies_mask, cg_twu_mask_cause |
| F7.NEW.7 | `twu` | **PTW PMP 端口 `mmu_pmp_fecth` 恒为 0（v6.0 新增：RTL twu.sv/ptw.sv 精读）** | PTW 通过 TWU 发起的 PMP 检查属于**数据读取**（Load PTE from memory），因此 `mmu_pmp_fecth`（RTL 信号名，注意 typo）对所有 4 个 PTW PMP 端口（pa3/5/6/7）恒为 0。`pmp_mmu_flg[2]`（X bit）在 `pmp_deny` 计算中用于 fetch type，PTW 不使用该路径，而是检查 `pmp_mmu_flg[0]`（R bit，因为 PTE 是 Load 操作）。验证关注点：（1）在所有 PTW PMP 检查场景下，`mmu_pmp_fecth{3,5,6,7}` 均为 0；（2）PMP 模块接收 fetch=0 后，使用 R-bit 而非 X-bit 进行权限判断；（3）若 R=0 且 fetch=0（数据访问），则产生 Access Fault（交叉 F7.NEW.5）。 | [twu.sv#L1001-1014](mmu/rtl/twu.sv#L1001), [ptw.sv#L61-64](mmu/rtl/ptw.sv#L61) | P1 | TC-PTW-PMP-FETCH-ZERO-001, TC-PTW-PMP-R-CHECK-001 | sva_ptw_pmp_fetch_zero |
