module mmu_l2tlb#(
    ...
    input  logic [TRANS_ID_WIDTH-1:0] ptw_l2tlb_ref_id,

    // PTW Ready (Needed for Miss Buffer)
    input  logic                     ptw_ready,

    output logic [TRANS_ID_WIDTH-1:0]l2tlb_ptw_id,      // Needed for PTW to track transaction,l2tlb miss buffer entry id
    ...
    logic                       mb_issue_req;
    logic [TRANS_ID_WIDTH-1:0]  mb_issue_eid;      // This is the Reqq ID
    ...
    assign l2tlb_tag_wen  = {WAY_NUM{arb_l2tlb_write}} & arb_l2tlb_way_sel;
    assign l2tlb_data_wen = {WAY_NUM{arb_l2tlb_write}} & arb_l2tlb_way_sel;
    ...
    always_ff@(posedge l2tlb_clk or negedge cpurst_b) begin
        if(!cpurst_b) begin
            raw_vld      <=     1'b0;
            raw_vpn      <=     {VPN_WIDTH'b0};
            //raw_trans_id <=     {TRANS_ID_WIDTH'b0};
            raw_tag      <=     {TAG_WIDTH'b0};
            raw_eid      <=     {EID_WIDTH'b0};
            raw_queue_id <=     {TRANS_ID_WIDTH'b0};
            raw_bank_index<=     '{default:0};
            raw_way_mask <=     {WAY_NUM'b0};
    ...
    always_ff@(posedge l2tlb_clk or negedge cpurst_b) begin
        if(!cpurst_b) begin
            final_vld      <=     1'b0;
            final_vpn      <=     {VPN_WIDTH'b0};
            //final_trans_id <=     {TRANS_ID_WIDTH'b0};
            final_tag      <=     {TAG_WIDTH'b0};
            final_eid      <=     {EID_WIDTH'b0};
            final_queue_id <=     {TRANS_ID_WIDTH'b0};
    ...
    mmu_l2tlb_mb#(
        ...
        .ACC_TPYE_WIDTH (TYPE_WIDTH) // Note: Fixed Typo in parameter name if possible
    ) x_l2tlb_mb (
        // Global
        .cp0_mmu_icg_en     (cp0_mmu_icg_en),
        .cpurst_b           (cpurst_b),
        ...
        .fb_trans_id        (ptw_l2tlb_ref_id),    // Loopback or from PTW if supported
