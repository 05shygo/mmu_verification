...
generate
    ...
    for (genvar l1dtlb_ent = 0; l1dtlb_ent < NUM_ENTRY; l1dtlb_ent = l1dtlb_ent + 1) begin: u_l1dtlb_ent_0_15
        
        //----------------------------------------------------------
        // Invalidation Logic (VA Match)
        //----------------------------------------------------------
        // Check if inv_va request matches this entry's VPN (partial match on bit [7:0] for timing optimization?)
        // Note: Ensure lsu_mmu_tlb_va is compared against valid entries
        assign ctc_inv_va_hit_clr[l1dtlb_ent] = tlboper_utlb_inv_va_req
                                              && l1dtlb_ent_vld[l1dtlb_ent] // Must be valid to be cleared
                                              && (lsu_mmu_tlb_va[7:0] == l1dtlb_ent_vpn[l1dtlb_ent][7:0]); 
                                              // Warning: Partial compare might over-flush, verify if intended.
        ...
        // Clock Gating
        //----------------------------------------------------------
        // Enable clock if: Global Clear OR Update this entry
        assign l1dtlb_ent_clk_en[l1dtlb_ent] = l1dtlb_entry_gating_clr
                                             | entry_upd[l1dtlb_ent];
        ...
        //!************************************************************** //! L1DTLB Entry Storage
        //!   1. l1dtlb_entry_vld
        //!   2. l1dtlb_entry_vpn[VPN_WIDTH-1:0]
        //!   3. l1dtlb_entry_ppn[PPN_WIDTH-1:0]
        //!   4. l1dtlb_entry_flg[FLG_WIDTH-1:0]
        //!************************************************************** // Valid Bit
        always @(posedge l1dtlb_entry_clk[l1dtlb_ent] or negedge cpurst_b) begin
          if(!cpurst_b)
            l1dtlb_ent_vld[l1dtlb_ent] <= 1'b0;
          else if(l1dtlb_entry_clr[l1dtlb_ent]) //! Clear Valid
            l1dtlb_ent_vld[l1dtlb_ent] <= 1'b0;
          else if(entry_upd[l1dtlb_ent])        //! Set Valid on Update
            l1dtlb_ent_vld[l1dtlb_ent] <= 1'b1;
        end
        
        // Payload (VPN, PPN, Flags)
        // Note: No reset needed for payload to save area, but good for simulation cleanliness
        always @(posedge l1dtlb_entry_clk[l1dtlb_ent] or negedge cpurst_b) begin
            if(!cpurst_b) begin
                l1dtlb_ent_vpn[l1dtlb_ent][VPN_WIDTH-1:0] <= {VPN_WIDTH{1'b0}};
                l1dtlb_ent_ppn[l1dtlb_ent][PPN_WIDTH-1:0] <= {PPN_WIDTH{1'b0}};
                ...
                // Update from Install Module Bus
                l1dtlb_ent_vpn[l1dtlb_ent][VPN_WIDTH-1:0] <= utlb_upd_vpn[VPN_WIDTH-1:0];
                l1dtlb_ent_ppn[l1dtlb_ent][PPN_WIDTH-1:0] <= utlb_upd_ppn[PPN_WIDTH-1:0];
                l1dtlb_ent_flg[l1dtlb_ent][FLG_WIDTH-1:0] <= utlb_upd_flg[FLG_WIDTH-1:0];
            end
        ...
        //!************************************************************** //! L1DTLB Hit Logic (CAM)
        //!************************************************************** // Port 0 Lookup
        assign l1dtlb_vpn_match0[l1dtlb_ent] = l1dtlb_ent_vld[l1dtlb_ent] 
                                             && (utlb_req_vpn0[VPN_WIDTH-1:0] == l1dtlb_ent_vpn[l1dtlb_ent][VPN_WIDTH-1:0]);
        assign entry_hit0[l1dtlb_ent]        = l1dtlb_vpn_match0[l1dtlb_ent];

        // Port 1 Lookup
        assign l1dtlb_vpn_match1[l1dtlb_ent] = l1dtlb_ent_vld[l1dtlb_ent] 
