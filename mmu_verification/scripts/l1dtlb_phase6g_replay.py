...
def run_row(args, row):
    ...
    print("=== phase6g replay: %s %s seed=%s status=%s ===" % (row.case_id, row.test, row.seed, row.status))
    if args.dry_run:
...
def write_summary(path, manifest, statuses, results, dry_run):
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write("L1DTLB Phase6G manifest replay summary\n")
...
def build_parser():
    parser = argparse.ArgumentParser(description="Replay L1DTLB Phase6G evidence manifest rows")
    ...
    parser.add_argument("--statuses", default="closure,future_formal", help="Manifest statuses to replay")
...
def main():
    ...
    for row in rows:
        ...
        if args.fail_fast and not result.passed:
            print("=== phase6g replay fail-fast: %s seed=%s rc=%d ===" % (row.test, row.seed, result.rc))
            break

    write_summary(summary, manifest, statuses, results, args.dry_run)
    passed = sum(1 for item in results if item.passed)
    failed = len(results) - passed
    print("L1DTLB Phase6G replay: PASS=%d FAIL=%d TOTAL=%d" % (passed, failed, len(results)))
