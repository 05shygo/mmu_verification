#!/usr/bin/env python3
"""PTW stage-8 regression/signoff gate.

The gate freezes the PTW source-side signoff package.  It is stricter for P0
source tests, keeps P2 illegal/constraint tests isolated, and treats consumer
...
def validate_closure_csv(rows: list[dict[str, str]]) -> list[str]:
    ...
    expected = (
        ...
        + required_ids("PTW-FLOW", 1, 23)
        + required_ids("PTW-INFRA", 1, 9)
        + required_ids("PDE-TP", 1, 12)
    ...
    for flow_id in required_ids("PTW-FLOW", 1, 23):
...
def main() -> int:
    parser = argparse.ArgumentParser(description="PTW stage-8 signoff gate")
    ...
    for role, list_path, seed in [
        ...
        ("p1_directed", args.p1_list, args.stage7_seed),
    ...
    if errors:
        print("PTW_STAGE8_SIGNOFF_GATE status=FAIL")
        for item in errors:
            print(f"  - {item}")
        return 1

    print(
        "PTW_STAGE8_SIGNOFF_GATE status=PASS "
        + " ".join(f"{role}_tests={count}" for role, count in sorted(counts.items()))
        + " source_sb=clean p0_cover=hit closure_matrix=frozen report=validated"
    )
    return 0
