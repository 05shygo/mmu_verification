...
module mmu_pde_cache_sva #(
  ...
  a_pde_consecutive_refill_update_onehot_known: assert property (@(posedge pde_cache_clk)
    disable iff (`L2TLB_NEG_DISABLE || !pde_past_valid)
  ...
  // PTW-SVA-PDE-015: L1 PDE update only stores l1pmpflg (guaranteed by RTL wiring).
  // l2pmpflg carries current-level PMP result (pmp_mmu_flg) which is non-zero for
  // L1 (scd-level) updates -- the check l2pmpflg==0 was incorrect and removed.
