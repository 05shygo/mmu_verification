...
module mmu_sva (
  ...
  a_ifu_va_known: assert property (@(posedge forever_cpuclk) disable iff (`L2TLB_NEG_DISABLE)
    ifu_mmu_va_vld |-> (!$isunknown(ifu_mmu_va) && ! $isunknown(ifu_mmu_abort)));

  a_lsu0_va_known: assert property (@(posedge forever_cpuclk) disable iff (`L2TLB_NEG_DISABLE)
    lsu_mmu_va0_vld |-> (!$isunknown(lsu_mmu_va0) && ! $isunknown(lsu_mmu_abort0)));

  a_lsu1_va_known: assert property (@(posedge forever_cpuclk) disable iff (`L2TLB_NEG_DISABLE)
    lsu_mmu_va1_vld |-> (!$isunknown(lsu_mmu_va1) && ! $isunknown(lsu_mmu_abort1)));

  a_lsu2_va_known: assert property (@(posedge forever_cpuclk) disable iff (`L2TLB_NEG_DISABLE)
    lsu_mmu_va2_vld |-> !$isunknown(lsu_mmu_va2));

  // 验证意图: 协议级 — 同周期 stall 与 PA 有效互斥（仅 pipe0/1；pipe2 在顶层无 stall 输出）
  a_lsu0_stall_excl_pavld: assert property (@(posedge forever_cpuclk) disable iff (`L2TLB_NEG_DISABLE)
    !(mmu_lsu_stall0 && mmu_lsu_pa0_vld));

  a_lsu1_stall_excl_pavld: assert property (@(posedge forever_cpuclk) disable iff (`L2TLB_NEG_DISABLE)
