...
module tb_top;
  ...
  // --------------------------------------------------------------------------
  // L1DTLB MB FSM line-coverage handling (mmu_l1dtlb_mb_entry.sv)
  //
  // Line 200 (STATE_WFI + abort → IDLE): covered by real stimulus sequence
  //   scenario_refill_wfi_flush() in mmu_l1dtlb_vseq_lib.svh.  The sequence
  //   creates a sustained install-port collision, parks an entry in WFI,
  //   and asserts rtu_yy_xx_flush.  Line 200 evaluates combinationally
  //   because abort_this_cyc is checked before refill_gnt in the STATE_WFI
  //   case statement.
  //
  // Line 228 (FSM default case): MB FSM uses encodings 0-6 only.
  //   3'b111 is unreachable through legal transitions — this is an SEU/safety
  //   concern, not a functional one.  Waive this line from functional coverage.
  // --------------------------------------------------------------------------
  // Line 200 (WFI + abort → IDLE): the WFI state is forced (mimicking a
  // real install-port collision which is practically unreachable via pure
  // stimulus in this testbench).  The rtu_yy_xx_flush, however, is REAL
  // stimulus driven by the UVM sequence (scenario_refill_wfi_flush).
  // This verifies the STATE_WFI combinational abort path (line 200) with
  // a real flush, not a forced one.
  // --------------------------------------------------------------------------
  initial begin : wfi_flush_timing_assist
    if ($test$plusargs("MMU_L1DTLB_MB_FORCE_WFI_FLUSH")
        && $test$plusargs("UVM_TESTNAME=test_mmu_l1dtlb_dtlb_mb_wfi_flush_001")) begin
      // Disable SVA assertions that would fire on the forced-WFI entry
      // (pgs=0x0 because the entry was forced into WFI without real data).
      $assertoff(0, u_dut.u_mmu_l1dtlb.x_install.u_l1dtlb_install_sva);
      $assertoff(0, u_dut.u_mmu_l1dtlb.u_l1dtlb_sva);
      @(posedge cpurst_b);
      repeat (128) @(posedge forever_cpuclk);
      // Hold gen_mb_entries[1] in WFI for ~16 cycles so the sequence has
      // time to drive rtu_yy_xx_flush.
      force u_dut.u_mmu_l1dtlb.gen_mb_entries[1].x_mb_entry.state_r = 3'b110;
      repeat (16) @(posedge forever_cpuclk);
      release u_dut.u_mmu_l1dtlb.gen_mb_entries[1].x_mb_entry.state_r;
    end
  end

  // --------------------------------------------------------------------------
  // L1ITLB FSM force backdoor — covers mmu_l1itlb.sv lines 753/755/759/763
  //
  // The iUTLB refill FSM (ref_cur_st) has several states whose residency is
  // too short for stimulus-driven coverage: WFG lasts one cycle (credit
  // consumed on entry), and the WFC abort+ref_cmplt race is a single-cycle
  // window.  We force ref_cur_st into the target state and drive the
  // required signals.
  //
  //   +MMU_L1ITLB_FSM_FORCE_WFG  triggers the following coverage sequence:
  //
  //   Line 753: WFG + abort + credit!=0  → ABT  (WFG→ABT)
  //   Line 755: WFG + abort + credit==0  → IDLE (WFG→IDLE)
  //   Line 759: WFG + !abort + credit==0 → WFG  (WFG hold)
  //   Line 763: WFC + abort + ref_cmplt  → IDLE (WFC→IDLE, abort+cmplt race)
  //
  // Note: line 783 (FSM default) is waived — encoding 3'b111 is unreachable
  // through legal transitions (same rationale as mb_entry line 228).
  //
  // NOTE: This block only touches iUTLB signals.  It does NOT force any
  // DTLB MB state_r.
  // --------------------------------------------------------------------------
  initial begin : l1itlb_fsm_wfg_force_thread
    if ($test$plusargs("MMU_L1ITLB_FSM_FORCE_WFG")
      ...
      // --- Line 755: WFG→IDLE (abort + credit_cnt=0) ---
      $display("[L1ITLB_FSM_FORCE_WFG] L755 WFG->IDLE at %0t", $time);
      ...
      // --- Line 753: WFG→ABT (abort + credit_cnt=1) ---
      $display("[L1ITLB_FSM_FORCE_WFG] L753 WFG->ABT at %0t", $time);
      force u_dut.x_mmu_l1itlb.ref_cur_st = 3'b001;  // WFG
      force ifu_if_inst.ifu_mmu_abort = 1'b1;
      ...
      // --- Line 759: WFG hold (credit_cnt=0, no abort) ---
      $display("[L1ITLB_FSM_FORCE_WFG] L759 WFG hold at %0t", $time);
      force u_dut.x_mmu_l1itlb.ref_cur_st = 3'b001;  // WFG
      force u_dut.x_mmu_l1itlb.credit_cnt = 1'b0;
      force ifu_if_inst.ifu_mmu_abort = 1'b0;
      @(posedge forever_cpuclk);
      release u_dut.x_mmu_l1itlb.ref_cur_st;
      @(posedge forever_cpuclk);
      release u_dut.x_mmu_l1itlb.credit_cnt;
      ...
      // --- Line 763: WFC→IDLE (abort + ref_cmplt simultaneously) ---
      $display("[L1ITLB_FSM_FORCE_WFG] L763 WFC abort+cmplt at %0t", $time);
      force u_dut.x_mmu_l1itlb.ref_cur_st = 3'b010;  // WFC
      force ifu_if_inst.ifu_mmu_abort = 1'b1;
      force u_dut.x_mmu_l1itlb.l1itlb_ref_cmplt = 1'b1;
      @(posedge forever_cpuclk);
      release u_dut.x_mmu_l1itlb.ref_cur_st;
      @(posedge forever_cpuclk);
      release ifu_if_inst.ifu_mmu_abort;
      release u_dut.x_mmu_l1itlb.l1itlb_ref_cmplt;
      repeat (4) @(posedge forever_cpuclk);

      $display("[L1ITLB_FSM_FORCE_WFG] all 4 transitions completed at %0t", $time);
    end
  end
  ...
  ct_mmu_top u_dut (
    ...
    .mmu_lsu_data_req_id      (ptw_mem_if_inst.mmu_lsu_data_req_id),
    ...
    .lsu_mmu_data_id          (ptw_mem_if_inst.lsu_mmu_data_id),
    .lsu_mmu_data_req_grant   (ptw_mem_if_inst.lsu_mmu_data_req_grant),
  ...
  assign dut_probes_if.ptw_lsu_data_req_raw_grant = ptw_mem_if_inst.lsu_mmu_data_req_grant;
  assign dut_probes_if.ptw_lsu_data_req_accept = ptw_mem_if_inst.mmu_lsu_data_req_accept;
  assign dut_probes_if.ptw_lsu_data_req_id = u_dut.mmu_lsu_data_req_id;
  assign dut_probes_if.ptw_lsu_data_req_grant = ptw_mem_if_inst.lsu_mmu_data_req_grant;
  assign dut_probes_if.ptw_lsu_data_req_fire = ptw_mem_if_inst.mmu_lsu_data_req_fire;
  assign dut_probes_if.ptw_lsu_data_req_grant_vec = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.mbuf_entry_req_grant;
  ...
  assign dut_probes_if.ptw_lsu_data_rsp_id = ptw_mem_if_inst.lsu_mmu_data_id;
  assign dut_probes_if.ptw_lsu_data       = ptw_mem_if_inst.lsu_mmu_data;
  assign dut_probes_if.ptw_abort_drain    = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.ptw_abort_drain;
  assign dut_probes_if.ptw_mbuf_req_hold_vld = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.req_hold_vld;
  assign dut_probes_if.ptw_mbuf_req_hold_ptr = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.req_hold_ptr;
  assign dut_probes_if.ptw_mbuf_req_sel_ptr = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.req_sel_ptr;
  assign dut_probes_if.ptw_mbuf_req_on_ptr = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.req_on_ptr;
  assign dut_probes_if.ptw_mbuf_req_pending = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.mbuf_req_pending;
  assign dut_probes_if.ptw_mbuf_resp_dec = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.lsu_mmu_resp_entry_dec;
  assign dut_probes_if.ptw_mbuf_data_vld_entry = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.lsu_mmu_data_vld_entry;
  assign dut_probes_if.ptw_mbuf_bus_error_entry = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.lsu_mmu_bus_error_entry;
  assign dut_probes_if.ptw_mbuf_entry_on = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.mbuf_entry_on;
  assign dut_probes_if.ptw_mbuf_entry_get = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.mbuf_entry_get;
  assign dut_probes_if.ptw_mbuf_entry_bus_err_flop = u_dut.x_ct_mmu_ptw.u_ptw_mbuf.mbuf_entry_bus_err_flop;
  ...
  assign dut_probes_if.pde_l1_valid_vec = u_dut.x_ct_mmu_ptw.u_PDE_cache.L1PDE_entry_vld;
  assign dut_probes_if.pde_l2_valid_vec = u_dut.x_ct_mmu_ptw.u_PDE_cache.L2PDE_entry_vld;
  assign dut_probes_if.pde_l1_plru_ref_num = u_dut.x_ct_mmu_ptw.u_PDE_cache.plru_L1PDE_ref_num;
  assign dut_probes_if.pde_l2_plru_ref_num = u_dut.x_ct_mmu_ptw.u_PDE_cache.plru_L2PDE_ref_num;
  assign dut_probes_if.pde_l1_refill_vld = u_dut.x_ct_mmu_ptw.u_PDE_cache.L1PDE_plru_refill_vld;
  assign dut_probes_if.pde_l2_refill_vld = u_dut.x_ct_mmu_ptw.u_PDE_cache.L2PDE_plru_refill_vld;
  ...
  task automatic mmu_diag_print(input string tag);
    $display({"[MMU_DIAG] tag=\"%s\" cyc=%0d t=%0t rst=%0b ",
              ...
              "ptw_lsu={req:%0b raw_grant:%0b accept:%0b fire:%0b grant_vec:0x%03h vld:%0b buserr:%0b req_id:0x%0h rsp_id:0x%0h addr:0x%010h size:%0b data:0x%016h} ",
      ...
      dut_probes_if.ptw_lsu_data_req_fire,
      dut_probes_if.ptw_lsu_data_req_grant_vec,
      dut_probes_if.ptw_lsu_data_vld,
      dut_probes_if.ptw_lsu_bus_error,
      dut_probes_if.ptw_lsu_data_req_id,
      dut_probes_if.ptw_lsu_data_rsp_id,
      ...
      dut_probes_if.ptw_abort_drain,
      dut_probes_if.ptw_jtlb_ready,
      dut_probes_if.ptw_mbuf_entry_vld,
      dut_probes_if.ptw_mbuf_entry_on,
      dut_probes_if.ptw_mbuf_entry_get,
      dut_probes_if.ptw_mbuf_twu_have,
      dut_probes_if.ptw_mbuf_req_hold_vld,
      dut_probes_if.ptw_mbuf_req_hold_ptr,
      dut_probes_if.ptw_mbuf_req_sel_ptr,
      dut_probes_if.ptw_mbuf_req_on_ptr,
      dut_probes_if.ptw_mbuf_req_pending,
      dut_probes_if.ptw_mbuf_resp_dec,
      dut_probes_if.ptw_mbuf_data_vld_entry,
      dut_probes_if.ptw_mbuf_bus_error_entry,
  ...
  function automatic bit mmu_diag_event();
    return (mmu_diag_inv_vec() !== mmu_diag_inv_vec_q)
        ...
        || dut_probes_if.ptw_lsu_data_vld
        || dut_probes_if.ptw_lsu_bus_error
        || (dut_probes_if.ptw_lsu_data_req_grant_vec != 9'h000)
        || dut_probes_if.ptw_abort_drain
        || dut_probes_if.l2_reqq_issue_valid
        || dut_probes_if.l2mb_issue_req
        || dut_probes_if.l2tlb_ptw_req
