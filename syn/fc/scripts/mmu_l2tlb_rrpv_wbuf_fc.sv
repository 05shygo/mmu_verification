        ...
        for (int k = 0; k < DEPTH; k++) begin
            ...
            if (valid_entry) begin
                for (int w = 0; w < WAY_NUM; w++) begin
                    if (buffer[ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)+w] && (buffer[ptr][WAY_NUM*RRPV_WIDTH+w*IDX_WIDTH +: IDX_WIDTH] == push_idx[w])) begin
            ...
            if (push_accept) begin
                for (int w = 0; w < WAY_NUM; w++) begin
                    if (push_hit_comb[w]) begin
                        buffer[push_hit_ptr[w]][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)+w]  <= push_vld[w];
                        buffer[push_hit_ptr[w]][WAY_NUM*RRPV_WIDTH+w*IDX_WIDTH +: IDX_WIDTH]  <= push_idx[w];
                        buffer[push_hit_ptr[w]][w*RRPV_WIDTH +: RRPV_WIDTH] <= push_data[w];
                    end
                end
            end
            ...
            if (push_accept && push_new_entry) begin
                for (int w = 0; w < WAY_NUM; w++) begin
                    if (push_new_bank[w]) begin
                        buffer[wr_ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)+w]  <= push_vld[w];
                        buffer[wr_ptr][WAY_NUM*RRPV_WIDTH+w*IDX_WIDTH +: IDX_WIDTH]  <= push_idx[w];
                        buffer[wr_ptr][w*RRPV_WIDTH +: RRPV_WIDTH] <= push_data[w];
                    end
                    else begin
                        buffer[wr_ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)+w] <= 1'b0;
                    end
                end
		...
		ptr = rd_ptr + k - DEPTH; // Wrap around adjustment
            ...
            if (valid_entry) begin
                ...
                for (int w = 0; w < WAY_NUM; w++) begin
                    ...
                    if (buffer[ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)+w] && (buffer[ptr][WAY_NUM*RRPV_WIDTH+w*IDX_WIDTH +: IDX_WIDTH] == lookup_idx[w])) begin
                        bypassed_rrpv_rdata_comb[w] = buffer[ptr][w*RRPV_WIDTH +: RRPV_WIDTH];
