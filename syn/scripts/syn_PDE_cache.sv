...
logic [(L1PDE_ENTRY_NUM-1-(0)+1)*(PPN_WIDTH-1-(0)+1)-1:0] L1PDE_entry_ppn        ;
logic [L1PDE_ENTRY_NUM-1:0]                L1PDE_entry_vld        ;
logic [L1PDE_ENTRY_NUM-1:0]                L1PDE_entry_hit        ;
logic [(L2PDE_ENTRY_NUM-1-(0)+1)*(PPN_WIDTH-1-(0)+1)-1:0] L2PDE_entry_ppn        ;
...
generate
	for(genvar L1PDE_ent = 0;L1PDE_ent < L1PDE_ENTRY_NUM;L1PDE_ent = L1PDE_ent + 1)begin:u_L1PDE_ent
		...
		.L1PDE_entry_ppn				(L1PDE_entry_ppn[(L1PDE_ent)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)] ),
...
generate
	for(genvar L2PDE_ent = 0;L2PDE_ent < L2PDE_ENTRY_NUM;L2PDE_ent = L2PDE_ent + 1)begin:u_L2PDE_ent
		...
		.L2PDE_entry_ppn				(L2PDE_entry_ppn[(L2PDE_ent)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)] ),
...
always_comb begin
	...
	for(int i = 0; i < L1PDE_ENTRY_NUM; i = i + 1) begin
		if(L1PDE_entry_hit_idx[i])
			L1PDE_cache_hit_ppn[PPN_WIDTH-1:0] = L1PDE_entry_ppn[(i)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)];
	end
end
...
always_comb begin
	...
	for(int i = 0; i < L2PDE_ENTRY_NUM; i = i + 1) begin
		if(L2PDE_entry_hit_idx[i])
			L2PDE_cache_hit_ppn[PPN_WIDTH-1:0] = L2PDE_entry_ppn[(i)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)];
	end
end
