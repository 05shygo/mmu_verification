...
logic [(NUM_ENTRY-1-(0)+1)*(FLG_WIDTH-1-(0)+1)-1:0] entry_flg;
logic [NUM_ENTRY-1:0]               entry_hit0, entry_hit1;
logic [(NUM_ENTRY-1-(0)+1)*(PPN_WIDTH-1-(0)+1)-1:0] entry_ppn;
logic [(NUM_ENTRY-1-(0)+1)*(PGS_WIDTH-1-(0)+1)-1:0] l1dtlb_ent_pgs;
...
logic [(MB_DEPTH-1-(0)+1)*(2-(0)+1)-1:0] mb_entry_state;
logic [(MB_DEPTH-1-(0)+1)*(VPN_WIDTH-1-(0)+1)-1:0] mb_entry_vpn;
logic [(MB_DEPTH-1-(0)+1)*(IID_WIDTH-1-(0)+1)-1:0] mb_entry_iid;
logic [(MB_DEPTH-1-(0)+1)*(PGS_WIDTH-1-(0)+1)-1:0] mb_entry_pgs;
...
assign expt_wr0_vld   = ptw_l1dtlb_ref_cmplt && (ptw_l1tlb_pgflt || ptw_l1tlb_acc_err)
                      ...
                      && (mb_entry_state[(ptw_l1dtlb_ref_id)*(2-(0)+1)+0 +: (2-(0)+1)] == MB_STATE_WFC)
                      && !rtu_yy_xx_flush;
assign expt_wr0_eid   = ptw_l1dtlb_ref_id[EID_WIDTH-1:0];
assign expt_wr0_iid   = mb_entry_iid[(ptw_l1dtlb_ref_id)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)];
assign expt_wr0_vpn   = mb_entry_vpn[(ptw_l1dtlb_ref_id)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
...
assign expt_wr1_vld   = jtlb_dutlb_ref_cmplt && jtlb_dutlb_pgflt
                      ...
                      && (mb_entry_state[(jtlb_dutlb_ref_id)*(2-(0)+1)+0 +: (2-(0)+1)] == MB_STATE_WFC)
                      && !rtu_yy_xx_flush;
assign expt_wr1_eid   = jtlb_dutlb_ref_id[EID_WIDTH-1:0];
assign expt_wr1_iid   = mb_entry_iid[(jtlb_dutlb_ref_id)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)];
assign expt_wr1_vpn   = mb_entry_vpn[(jtlb_dutlb_ref_id)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
...
always @(posedge mb_clk) begin
  ...
  if (ptw_l1dtlb_ref_cmplt) begin
    key = $sformatf("R_PTW_ref%0d_iid%0d_vpn%0h", ptw_l1dtlb_ref_id,
                    mb_entry_iid[(ptw_l1dtlb_ref_id)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)], mb_entry_vpn[(ptw_l1dtlb_ref_id)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);
    if (!seen_ref.exists(key)) begin
      ...
      $display("[MMU_EXPT_TRACE_ONCE][REF] t=%0t src=ptw ref_id=%0d iid=%0d vpn=0x%0h pgflt=%0b acerr=%0b mb_vld=%0b",
        $time, ptw_l1dtlb_ref_id, mb_entry_iid[(ptw_l1dtlb_ref_id)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)], mb_entry_vpn[(ptw_l1dtlb_ref_id)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)],
  ...
  if (jtlb_dutlb_ref_cmplt) begin
    key = $sformatf("R_JTLB_ref%0d_iid%0d_vpn%0h", jtlb_dutlb_ref_id,
                    mb_entry_iid[(jtlb_dutlb_ref_id)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)], mb_entry_vpn[(jtlb_dutlb_ref_id)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);
    if (!seen_ref.exists(key)) begin
      seen_ref[key] = 1'b1;
      $display("[MMU_EXPT_TRACE_ONCE][REF] t=%0t src=jtlb ref_id=%0d iid=%0d vpn=0x%0h pgflt=%0b mb_vld=%0b",
        $time, jtlb_dutlb_ref_id, mb_entry_iid[(jtlb_dutlb_ref_id)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)], mb_entry_vpn[(jtlb_dutlb_ref_id)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)],
...
generate
    for (k = 0; k < NUM_ENTRY; k++) begin : gen_pack_signals
        assign entry_flg_vec[k*FLG_WIDTH +: FLG_WIDTH] = entry_flg[(k)*(FLG_WIDTH-1-(0)+1)+0 +: (FLG_WIDTH-1-(0)+1)];
        assign entry_ppn_vec[k*PPN_WIDTH +: PPN_WIDTH] = entry_ppn[(k)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)];
    end
endgenerate
...
always_comb begin
    ...
    for (int k = 0; k < MB_DEPTH; k++) begin
        ...
        mb_hit0_vec[k] = miss0_vld_q && mb_entry_vld[k] && (mb_entry_vpn[(k)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)] == miss0_vpn_q);
    end
    mb_hit0 = |mb_hit0_vec; // OR reduction to get single hit signal

    // Port 1 Hit Detection
    for (int k = 0; k < MB_DEPTH; k++) begin
        mb_hit1_vec[k] = miss1_vld_q && mb_entry_vld[k] && (mb_entry_vpn[(k)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)] == miss1_vpn_q);
    end
    mb_hit1 = |mb_hit1_vec;
end
...
//!************************************************
logic [(MB_DEPTH-1-(0)+1)*(PPN_WIDTH-1-(0)+1)-1:0] mb_entry_ppn;
logic [(MB_DEPTH-1-(0)+1)*(FLG_WIDTH-1-(0)+1)-1:0] mb_entry_flg;
...
generate
	...
	logic [PGS_WIDTH-1:0]   entry_ref_pgs;
	    ...
	    .refill_pgs	   (entry_ref_pgs),
            ...
            .entry_state   (mb_entry_state[(i)*(2-(0)+1)+0 +: (2-(0)+1)]),
            .entry_vpn     (mb_entry_vpn[(i)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]),
            .entry_ppn     (mb_entry_ppn[(i)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)]),
            .entry_flg     (mb_entry_flg[(i)*(FLG_WIDTH-1-(0)+1)+0 +: (FLG_WIDTH-1-(0)+1)]),
            .entry_iid     (mb_entry_iid[(i)*(IID_WIDTH-1-(0)+1)+0 +: (IID_WIDTH-1-(0)+1)]),
	    .entry_pgs	   (mb_entry_pgs[(i)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)]),
...
logic [(15-(0)+1)*(VPN_WIDTH-1-(0)+1)-1:0] l1dtlb_ent_vpn;
logic [(15-(0)+1)*(PPN_WIDTH-1-(0)+1)-1:0] l1dtlb_ent_ppn;
logic [(15-(0)+1)*(FLG_WIDTH-1-(0)+1)-1:0] l1dtlb_ent_flg;
...
generate
    ...
    for (genvar l1dtlb_ent = 0; l1dtlb_ent < NUM_ENTRY; l1dtlb_ent = l1dtlb_ent + 1) begin: u_l1dtlb_ent
        ...
        assign ctc_inv_va_hit_clr[l1dtlb_ent] = tlboper_utlb_inv_va_req
                                              ...
                                              && (lsu_mmu_tlb_va[7:0] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]); 
        ...
        always @(posedge l1dtlb_entry_clk[l1dtlb_ent] or negedge cpurst_b) begin
            if(!cpurst_b) begin
                l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)] <= {VPN_WIDTH{1'b0}};
                l1dtlb_ent_ppn[(l1dtlb_ent)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)] <= {PPN_WIDTH{1'b0}};
                l1dtlb_ent_flg[(l1dtlb_ent)*(FLG_WIDTH-1-(0)+1)+0 +: (FLG_WIDTH-1-(0)+1)] <= {FLG_WIDTH{1'b0}};
                l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] <= {PGS_WIDTH{1'b0}};
            end
            else if(entry_upd[l1dtlb_ent]) begin
                l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)] <= utlb_upd_vpn[VPN_WIDTH-1:0];
                l1dtlb_ent_ppn[(l1dtlb_ent)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)] <= utlb_upd_ppn[PPN_WIDTH-1:0];
                l1dtlb_ent_flg[(l1dtlb_ent)*(FLG_WIDTH-1-(0)+1)+0 +: (FLG_WIDTH-1-(0)+1)] <= utlb_upd_flg[FLG_WIDTH-1:0];
                // Store the Page Size info (Critical for hybrid support)
                l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] <= utlb_refill_pgs[PGS_WIDTH-1:0]; 
            end
        end
        ...
        assign hit0_4k = (utlb_req_vpn0[VPN_WIDTH-1:0] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);
        // 2MB: Compare [26:9] (Ignore lower 9 bits)
        assign hit0_2m = (utlb_req_vpn0[VPN_WIDTH-1:LVL_WIDTH] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);
        // 1GB: Compare [26:18] (Ignore lower 18 bits)
        assign hit0_1g = (utlb_req_vpn0[VPN_WIDTH-1:2*LVL_WIDTH] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);

        // --- Port 1 Comparators ---
        assign hit1_4k = (utlb_req_vpn1[VPN_WIDTH-1:0] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);
        assign hit1_2m = (utlb_req_vpn1[VPN_WIDTH-1:LVL_WIDTH] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);
        assign hit1_1g = (utlb_req_vpn1[VPN_WIDTH-1:2*LVL_WIDTH] == l1dtlb_ent_vpn[(l1dtlb_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]);

        // --- Final Match Logic ---
        // Select the correct comparator result based on the stored Page Size (pgs)
        // Assuming encoding: pgs[0]=4K, pgs[1]=2M, pgs[2]=1G
        
        assign l1dtlb_vpn_match0[l1dtlb_ent] = (l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] & hit0_4k) 
                                             | (l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] & hit0_2m)
                                             | (l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] & hit0_1g);

        assign l1dtlb_vpn_match1[l1dtlb_ent] = (l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] & hit1_4k) 
                                             | (l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] & hit1_2m)
                                             | (l1dtlb_ent_pgs[(l1dtlb_ent)*(PGS_WIDTH-1-(0)+1)+0 +: (PGS_WIDTH-1-(0)+1)] & hit1_1g);
        ...
        assign entry_ppn[(l1dtlb_ent)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)] = l1dtlb_ent_ppn[(l1dtlb_ent)*(PPN_WIDTH-1-(0)+1)+0 +: (PPN_WIDTH-1-(0)+1)];
        assign entry_flg[(l1dtlb_ent)*(FLG_WIDTH-1-(0)+1)+0 +: (FLG_WIDTH-1-(0)+1)] = l1dtlb_ent_flg[(l1dtlb_ent)*(FLG_WIDTH-1-(0)+1)+0 +: (FLG_WIDTH-1-(0)+1)];
