module mmu_l1dtlb_expt_cam #(
  ...
  logic [IID_WIDTH+VPN_WIDTH+2:0] ent [CAM_DEPTH-1:0];
  ...
  always_comb begin
    for (j = 0; j < CAM_DEPTH; j++) begin
      hit0_vec[j]    = ent[j][IID_WIDTH+VPN_WIDTH+2] && (ent[j][IID_WIDTH+VPN_WIDTH+1:VPN_WIDTH+2] == lsu_mmu_id0);
      hit1_vec[j]    = ent[j][IID_WIDTH+VPN_WIDTH+2] && (ent[j][IID_WIDTH+VPN_WIDTH+1:VPN_WIDTH+2] == lsu_mmu_id1);
      hit0_use_vec[j]= hit0_vec[j] && (ent[j][VPN_WIDTH+1:2] == lsu_mmu_vpn0);
      hit1_use_vec[j]= hit1_vec[j] && (ent[j][VPN_WIDTH+1:2] == lsu_mmu_vpn1);
    end
  end
  ...
  always_comb begin
    ...
    if (consume0_eff) begin
      expt_pgflt0 = ent[hit0_idx][1];
      expt_acflt0 = ent[hit0_idx][0];
    end
    ...
    if (consume1_eff) begin
      expt_pgflt1 = ent[hit1_idx][1];
      expt_acflt1 = ent[hit1_idx][0];
    end
  end
  ...
  always_ff @(posedge clk or negedge rst_b) begin
    if (!rst_b) begin
      for (k = 0; k < CAM_DEPTH; k++) begin
        ent[k][IID_WIDTH+VPN_WIDTH+2]   <= 1'b0;
        ent[k][IID_WIDTH+VPN_WIDTH+1:VPN_WIDTH+2]   <= '0;
        ent[k][VPN_WIDTH+1:2]   <= '0;
        ent[k][1] <= 1'b0;
        ent[k][0] <= 1'b0;
      end
    end
      ...
      if (consume0_eff)
        ent[hit0_idx][IID_WIDTH+VPN_WIDTH+2] <= 1'b0;
      if (consume1_eff)
        ent[hit1_idx][IID_WIDTH+VPN_WIDTH+2] <= 1'b0;

      if (expt_wr1_vld && !same_wr_eid) begin
        ent[expt_wr1_eid][IID_WIDTH+VPN_WIDTH+2]   <= 1'b1;
        ent[expt_wr1_eid][IID_WIDTH+VPN_WIDTH+1:VPN_WIDTH+2]   <= expt_wr1_iid;
        ent[expt_wr1_eid][VPN_WIDTH+1:2]   <= expt_wr1_vpn;
        ent[expt_wr1_eid][1] <= expt_wr1_pgflt;
        ent[expt_wr1_eid][0] <= expt_wr1_acflt;
      end

      if (expt_wr0_vld) begin
        ent[expt_wr0_eid][IID_WIDTH+VPN_WIDTH+2]   <= 1'b1;
        ent[expt_wr0_eid][IID_WIDTH+VPN_WIDTH+1:VPN_WIDTH+2]   <= expt_wr0_iid;
        ent[expt_wr0_eid][VPN_WIDTH+1:2]   <= expt_wr0_vpn;
        ent[expt_wr0_eid][1] <= expt_wr0_pgflt;
        ent[expt_wr0_eid][0] <= expt_wr0_acflt;
      end
    end
  end
