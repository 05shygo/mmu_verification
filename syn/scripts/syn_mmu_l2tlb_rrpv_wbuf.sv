...
module mmu_l2tlb_rrpv_wbuf#(
    ...
    input  logic [(WAY_NUM-1-(0)+1)*(IDX_WIDTH-1-(0)+1)-1:0] push_idx, 
    ...
    input  logic [(WAY_NUM-1-(0)+1)*(RRPV_WIDTH-1-(0)+1)-1:0] push_data,
    ...
    output logic [(WAY_NUM-1-(0)+1)*(IDX_WIDTH-1-(0)+1)-1:0] sram_idx,
    output logic [(WAY_NUM-1-(0)+1)*(RRPV_WIDTH-1-(0)+1)-1:0] sram_data,

    //-------------------------------------------------------------------------
    // 3. Bypass Interface (Lookup Stage)
    //-------------------------------------------------------------------------
    // Current Lookup Indices from Hash Logic
    input  logic [(WAY_NUM-1-(0)+1)*(IDX_WIDTH-1-(0)+1)-1:0] lookup_idx,
    input  logic			      lookup_req,
    
    // Stale Data read from SRAM (potentially outdated)
    //input  logic [(WAY_NUM-1-(0)+1)*(RRPV_WIDTH-1-(0)+1)-1:0] rrpv_sram_rdata,
    
    // Final RRPV Data (Merged: SRAM Read + Pending Buffer Updates)
    output logic [(WAY_NUM-1-(0)+1)*(RRPV_WIDTH-1-(0)+1)-1:0] bypassed_rrpv_rdata,
    ...
    logic [WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:0] buffer [DEPTH-1:0];
    ...
    logic [(WAY_NUM-1-(0)+1)*(RRPV_WIDTH-1-(0)+1)-1:0] bypassed_rrpv_rdata_comb;
    logic [WAY_NUM-1:0] push_hit_comb;
    logic [(WAY_NUM-1-(0)+1)*($clog2(DEPTH)-1-(0)+1)-1:0] push_hit_ptr;
        ...
        for (int k = 0; k < DEPTH; k++) begin
            ...
            if (valid_entry) begin
                for (int w = 0; w < WAY_NUM; w++) begin
                    if (buffer[ptr][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)][w] && (buffer[ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*RRPV_WIDTH][w] == push_idx[w])) begin
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            for (int i = 0; i < DEPTH; i++) begin
                buffer[i][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)] <= '0;
            end
        end
        else begin
            // Pop Logic
            if (pop_do) begin
                buffer[rd_ptr][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)] <= '0;
                // Wrap around logic
            ...
            if (push_accept) begin
                for (int w = 0; w < WAY_NUM; w++) begin
                    if (push_hit_comb[w]) begin
                        buffer[push_hit_ptr[w]][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)][w]  <= push_vld[w];
                        buffer[push_hit_ptr[w]][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*RRPV_WIDTH][w]  <= push_idx[w];
                        buffer[push_hit_ptr[w]][WAY_NUM*RRPV_WIDTH-1:0][w] <= push_data[w];
                    end
                end
            end
            ...
            if (push_accept && push_new_entry) begin
                for (int w = 0; w < WAY_NUM; w++) begin
                    if (push_new_bank[w]) begin
                        buffer[wr_ptr][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)][w]  <= push_vld[w];
                        buffer[wr_ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*RRPV_WIDTH][w]  <= push_idx[w];
                        buffer[wr_ptr][WAY_NUM*RRPV_WIDTH-1:0][w] <= push_data[w];
                    end
                    else begin
                        buffer[wr_ptr][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)][w] <= 1'b0;
                    end
                end
    ...
    assign sram_vld  = buffer[rd_ptr][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)];
    assign sram_idx  = buffer[rd_ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*RRPV_WIDTH];
    assign sram_data = buffer[rd_ptr][WAY_NUM*RRPV_WIDTH-1:0];
		...
		ptr = rd_ptr + k - DEPTH; // Wrap around adjustment
            ...
            if (valid_entry) begin
                ...
                for (int w = 0; w < WAY_NUM; w++) begin
                    ...
                    if (buffer[ptr][WAY_NUM*(1+IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)][w] && (buffer[ptr][WAY_NUM*(IDX_WIDTH+RRPV_WIDTH)-1:WAY_NUM*RRPV_WIDTH][w] == lookup_idx[w])) begin
                        bypassed_rrpv_rdata_comb[w] = buffer[ptr][WAY_NUM*RRPV_WIDTH-1:0][w];
