...
//!******************************************
    ...
    input  logic [(3-(0)+1)*(PADDR_WIDTH-1-(0)+1)-1:0] twu_mbuf_paddr,
    input  logic [(3-(0)+1)*(VPN_WIDTH-1-(0)+1)-1:0] twu_mbuf_vpn,
    input  logic [(3-(0)+1)*(TYPE_WIDTH-1-(0)+1)-1:0] twu_mbuf_type,
    input  logic [(3-(0)+1)*(ID_WIDTH-1-(0)+1)-1:0] twu_mbuf_id,
    input  logic [(3-(0)+1)*(PTE_LEVEL-1-(0)+1)-1:0] twu_mbuf_lvl,
    input  logic [(3-(0)+1)*(3-(0)+1)-1:0] twu_mbuf_twu_idx,
    input  logic [(3-(0)+1)*(7-(0)+1)-1:0] twu_mbuf_pmpflg,
...
//!******************************************
    ...
    input  logic [(3-(0)+1)*(PTE_LEVEL-1-(0)+1)-1:0] twu_data_ready,
...
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(PADDR_WIDTH-1-(0)+1)-1:0] mbuf_entry_padder;
logic [MBUF_ENTRY_NUM-1:0]                  mbuf_entry_vld;
logic [MBUF_ENTRY_NUM-1:0]                  mbuf_entry_on;
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(VPN_WIDTH-1-(0)+1)-1:0] mbuf_entry_vpn;
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(TYPE_WIDTH-1-(0)+1)-1:0] mbuf_entry_type;
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(ID_WIDTH-1-(0)+1)-1:0] mbuf_entry_id;
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(3-(0)+1)-1:0] mbuf_entry_twu_idx;
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(PTE_LEVEL-1-(0)+1)-1:0] mbuf_entry_lvl;
//logic	[3:0]		 twu_have			;
//logic	[3:0]		mbuf_twu_idx			;
...
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(DATA_WIDTH-1-(0)+1)-1:0] mbuf_entry_data;
...
logic [(MBUF_ENTRY_NUM-1-(0)+1)*(7-(0)+1)-1:0] mbuf_entry_pmpflg;
...
//        if(mbuf_entry_twu_idx[(i)*(3-(0)+1)+0 +: (3-(0)+1)] == 4'b0001 && mbuf_entry_vld[i]) begin
//            twu_have[0] = 1'b1;
//        end
//        if(mbuf_entry_twu_idx[(i)*(3-(0)+1)+0 +: (3-(0)+1)] == 4'b0010 && mbuf_entry_vld[i]) begin
//            twu_have[1] = 1'b1;
//        end
//        if(mbuf_entry_twu_idx[(i)*(3-(0)+1)+0 +: (3-(0)+1)] == 4'b0100 && mbuf_entry_vld[i]) begin
//            twu_have[2] = 1'b1;
//        end
//        if(mbuf_entry_twu_idx[(i)*(3-(0)+1)+0 +: (3-(0)+1)] == 4'b1000 && mbuf_entry_vld[i]) begin
//            twu_have[3] = 1'b1;
//        end
 //   end
//end
...
//                  MBUF Upd Arbiter
//==============================================================================
assign fst_twu_itlb_sel = twu_mbuf_req[0] & (twu_mbuf_type[(0)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)] == 3'b011);
assign scd_twu_itlb_sel = twu_mbuf_req[1] & (twu_mbuf_type[(1)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)] == 3'b011);
assign thd_twu_itlb_sel = twu_mbuf_req[2] & (twu_mbuf_type[(2)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)] == 3'b011);
assign fth_twu_itlb_sel = twu_mbuf_req[3] & (twu_mbuf_type[(3)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)] == 3'b011);
...
always_comb begin
	case(mbuf_grant[3:0])
		4'b1000	: begin
			mbuf_upd_padder[PADDR_WIDTH-1:0] = twu_mbuf_paddr[(3)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
            mbuf_upd_vpn[VPN_WIDTH-1:0] = twu_mbuf_vpn[(3)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
            mbuf_upd_type[TYPE_WIDTH-1:0] = twu_mbuf_type[(3)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)];
            mbuf_upd_id[ID_WIDTH-1:0] = twu_mbuf_id[(3)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)];
            mbuf_upd_twu_idx[3:0] = twu_mbuf_twu_idx[(3)*(3-(0)+1)+0 +: (3-(0)+1)];
            mbuf_upd_lvl[PTE_LEVEL-1:0] = twu_mbuf_lvl[(3)*(PTE_LEVEL-1-(0)+1)+0 +: (PTE_LEVEL-1-(0)+1)];
            mbuf_upd_pmpflg[7:0] = twu_mbuf_pmpflg[(3)*(7-(0)+1)+0 +: (7-(0)+1)];
		end
		4'b0100 : begin
			mbuf_upd_padder[PADDR_WIDTH-1:0] = twu_mbuf_paddr[(2)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
            mbuf_upd_vpn[VPN_WIDTH-1:0] = twu_mbuf_vpn[(2)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
            mbuf_upd_type[TYPE_WIDTH-1:0] = twu_mbuf_type[(2)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)];
            mbuf_upd_id[ID_WIDTH-1:0] = twu_mbuf_id[(2)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)];
            mbuf_upd_twu_idx[3:0] = twu_mbuf_twu_idx[(2)*(3-(0)+1)+0 +: (3-(0)+1)];
            mbuf_upd_lvl[PTE_LEVEL-1:0] = twu_mbuf_lvl[(2)*(PTE_LEVEL-1-(0)+1)+0 +: (PTE_LEVEL-1-(0)+1)];
            mbuf_upd_pmpflg[7:0] = twu_mbuf_pmpflg[(2)*(7-(0)+1)+0 +: (7-(0)+1)];
		end
		4'b0010 : begin
			mbuf_upd_padder[PADDR_WIDTH-1:0] = twu_mbuf_paddr[(1)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
            mbuf_upd_vpn[VPN_WIDTH-1:0] = twu_mbuf_vpn[(1)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
            mbuf_upd_type[TYPE_WIDTH-1:0] = twu_mbuf_type[(1)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)];
            mbuf_upd_id[ID_WIDTH-1:0] = twu_mbuf_id[(1)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)];
            mbuf_upd_twu_idx[3:0] = twu_mbuf_twu_idx[(1)*(3-(0)+1)+0 +: (3-(0)+1)];
            mbuf_upd_lvl[PTE_LEVEL-1:0] = twu_mbuf_lvl[(1)*(PTE_LEVEL-1-(0)+1)+0 +: (PTE_LEVEL-1-(0)+1)];
            mbuf_upd_pmpflg[7:0] = twu_mbuf_pmpflg[(1)*(7-(0)+1)+0 +: (7-(0)+1)];
		end
		4'b0001 : begin
			mbuf_upd_padder[PADDR_WIDTH-1:0] = twu_mbuf_paddr[(0)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
            mbuf_upd_vpn[VPN_WIDTH-1:0] = twu_mbuf_vpn[(0)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
            mbuf_upd_type[TYPE_WIDTH-1:0] = twu_mbuf_type[(0)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)];
            mbuf_upd_id[ID_WIDTH-1:0] = twu_mbuf_id[(0)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)];
            mbuf_upd_twu_idx[3:0] = twu_mbuf_twu_idx[(0)*(3-(0)+1)+0 +: (3-(0)+1)];
            mbuf_upd_lvl[PTE_LEVEL-1:0] = twu_mbuf_lvl[(0)*(PTE_LEVEL-1-(0)+1)+0 +: (PTE_LEVEL-1-(0)+1)];
            mbuf_upd_pmpflg[7:0] = twu_mbuf_pmpflg[(0)*(7-(0)+1)+0 +: (7-(0)+1)];
		end
		default : begin
...
always_comb begin
	...
	for(int i = 0; i < MBUF_ENTRY_NUM; i = i + 1) begin
		...
		mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(i)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
	    end
    end
end
...
////		assign mask[i] = (|(twu_mbuf_mask[3:0] & mbuf_entry_twu_idx[(i)*(3-(0)+1)+0 +: (3-(0)+1)])) & mbuf_entry_vld[i];
////	end
////endgenerate
//
//
...
//		9'b0_0000_0001	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(0)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_0000_0010	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(1)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_0000_0100	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(2)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_0000_1000	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(3)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_0001_0000	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(4)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_0010_0000	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(5)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_0100_0000	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(6)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b0_1000_0000	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(7)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
//		9'b1_0000_0000	:	mmu_lsu_data_req_addr[PADDR_WIDTH-1:0] = mbuf_entry_padder[(8)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)];
...
generate
	for(genvar MBUF_ent = 0;MBUF_ent < MBUF_ENTRY_NUM;MBUF_ent = MBUF_ent + 1)begin:u_MBUF_ent_0_8
		mbuf_entry #(
			...
			.mbuf_entry_padder			(mbuf_entry_padder[(MBUF_ent)*(PADDR_WIDTH-1-(0)+1)+0 +: (PADDR_WIDTH-1-(0)+1)]                 ),
			.mbuf_entry_vld				(mbuf_entry_vld[MBUF_ent]                    ),
			.mbuf_entry_on				(mbuf_entry_on[MBUF_ent]                     ),
			.mbuf_entry_vpn				(mbuf_entry_vpn[(MBUF_ent)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)]                    ),
			.mbuf_entry_type			(mbuf_entry_type[(MBUF_ent)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)]                   ),
			.mbuf_entry_id				(mbuf_entry_id[(MBUF_ent)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)]                     ),
			.mbuf_entry_twu_idx			(mbuf_entry_twu_idx[(MBUF_ent)*(3-(0)+1)+0 +: (3-(0)+1)]                ),
			.mbuf_entry_lvl             (mbuf_entry_lvl[(MBUF_ent)*(PTE_LEVEL-1-(0)+1)+0 +: (PTE_LEVEL-1-(0)+1)]                    ),
			.mbuf_entry_pmpflg			(mbuf_entry_pmpflg[(MBUF_ent)*(7-(0)+1)+0 +: (7-(0)+1)]				 ),
            .mbuf_entry_data            (mbuf_entry_data[(MBUF_ent)*(DATA_WIDTH-1-(0)+1)+0 +: (DATA_WIDTH-1-(0)+1)]                   ),
...
always_comb begin
    ...
    for (int i = 0; i < MBUF_ENTRY_NUM; i++) begin
        if(mbuf_bus_error_grant[i])begin
			entry_bus_err_type[TYPE_WIDTH-1:0] = mbuf_entry_type[(i)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)];
			entry_bus_err_id[ID_WIDTH-1:0] = mbuf_entry_id[(i)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)];
        end
    end
end
...
always_comb begin
    ...
    for (int i = 0; i < MBUF_ENTRY_NUM; i++) begin
        if(write_back_grant[i])begin
			mbuf_twu_vpn[VPN_WIDTH-1:0] = mbuf_entry_vpn[(i)*(VPN_WIDTH-1-(0)+1)+0 +: (VPN_WIDTH-1-(0)+1)];
			mbuf_twu_type[TYPE_WIDTH-1:0] = mbuf_entry_type[(i)*(TYPE_WIDTH-1-(0)+1)+0 +: (TYPE_WIDTH-1-(0)+1)];
			mbuf_twu_id[ID_WIDTH-1:0] = mbuf_entry_id[(i)*(ID_WIDTH-1-(0)+1)+0 +: (ID_WIDTH-1-(0)+1)];
			mbuf_twu_idx[3:0] = mbuf_entry_twu_idx[(i)*(3-(0)+1)+0 +: (3-(0)+1)];
			mbuf_twu_lvl[PTE_LEVEL-1:0] = mbuf_entry_lvl[(i)*(PTE_LEVEL-1-(0)+1)+0 +: (PTE_LEVEL-1-(0)+1)];
			mbuf_twu_pmpflg[7:0] = mbuf_entry_pmpflg[(i)*(7-(0)+1)+0 +: (7-(0)+1)];
            mbuf_twu_data[DATA_WIDTH-1:0] = mbuf_entry_data[(i)*(DATA_WIDTH-1-(0)+1)+0 +: (DATA_WIDTH-1-(0)+1)];
        end
    end
end
